...
function tagLabel(category: string) {
  switch (category) {
    case "Administration": return translate("actionBoard.tags.administration");
    case "Sports": return translate("actionBoard.tags.sports");
    case "Equipment": return translate("actionBoard.tags.equipment");
    case "Event": return translate("actionBoard.tags.event");
    case "Finance": return translate("actionBoard.tags.finance");
    case "Membership": return translate("actionBoard.tags.membership");
    case "Other": return translate("actionBoard.tags.other");
...
function defaultAnswerOptions() {
  return translate("actionBoard.decision.defaultAnswerOptions");
}

function defaultAnswerLabels() {
  return defaultAnswerOptions().split("\n");
}

function answerLabels(raw: string, fallback?: string[]) {
  const labels = [...new Set(raw.split(/\n/).map((value) => value.trim()).filter(Boolean))].slice(0, 8);
  return labels.length >= 2 ? labels : fallback ?? defaultAnswerLabels();
}
...
function actor(user: SharedRosterUser | null): TaskBoardPerson {
  return {
    name: user?.displayName?.trim() || user?.email || translate("actionBoard.people.organizerFallback"),
...
function createDefaultBoard(rosterName: string): LocalBoard {
  const now = Date.now();
  return {
    meta: { name: rosterName.trim() || translate("actionBoard.title"), createdAt: now, updatedAt: now },
...
function repeatDecisionLabel(decision?: TaskBoardVote) {
  ...
  if (decision.decisionType === "schedule" || decision.kind === "schedule") return translate("actionBoard.decision.anotherSchedule");
  if (decision.decisionType === "players") return translate("actionBoard.decision.anotherPlayerDecision");
  if (decision.decisionType === "equipment") return translate("actionBoard.decision.anotherEquipmentDecision");
  return null;
}
...
function dueText(value?: string) {
  ...
  return formatDateTime(getResolvedUiLocale(), date, { day: "numeric", month: "short" });
}
...
function activityText(activity: TaskBoardActivity) {
  const values = { actorName: activity.actorName };
  if (activity.action === "created") return translate("actionBoard.activity.created", values);
  if (activity.action === "vote_started") return translate("actionBoard.activity.voteStarted", values);
  if (activity.action === "vote_closed") return translate("actionBoard.activity.voteClosed", values);
  if (activity.action === "decision_recorded") return translate("actionBoard.activity.decisionRecorded", values);
  if (activity.action === "action_defined") return translate("actionBoard.activity.actionDefined", values);
  if (activity.action === "claimed") return translate("actionBoard.activity.claimed", values);
  if (activity.action === "released") return translate("actionBoard.activity.released", values);
  if (activity.action === "completed") return translate("actionBoard.activity.completed", values);
  if (activity.action === "link_added") return translate("actionBoard.activity.linkAdded", values);
  if (activity.action === "reopened") return translate("actionBoard.activity.reopened", values);
  return translate("actionBoard.activity.updated", values);
}
...
function providerLabel(url: string) {
  try {
    ...
    return translate("actionBoard.link.label");
  }
}
...
function voteOptionLabels(kind: TaskBoardVoteKind, raw: string) {
  if (kind === "yes-no-abstain") {
    return [
      translate("actionBoard.decision.answerOptionYes"),
      translate("actionBoard.decision.answerOptionNo"),
      translate("actionBoard.decision.answerOptionAbstain"),
    ];
  }
...
function googleCalendarUrl(card: TaskBoardCard, decision: TaskBoardVote) {
  ...
  const params = new URLSearchParams({
    ...
    details: decision.meetingUrl
      ? translate("actionBoard.calendar.meetingLink", { url: decision.meetingUrl })
      : translate("actionBoard.calendar.createdInStripes"),
...
function currentNeed(card: TaskBoardCard) {
  ...
  if (openDecision) {
    if (openDecision.decisionType === "schedule" || openDecision.kind === "schedule") {
      if (openDecision.scheduleState === "waiting-host") return translate("actionBoard.schedule.waitingForHost");
      if (openDecision.scheduleState === "setup") return translate("actionBoard.schedule.hostSelectedSetup");
      return openDecision.title?.trim() || translate("actionBoard.schedule.findTime");
    }
    if (openDecision.title?.trim()) return openDecision.title.trim();
    const questions = decisionQuestions(openDecision);
    if (questions.length > 1) return translate("actionBoard.decision.questionAndMore", { question: questions[0].text, count: questions.length - 1 });
    if (openDecision.decisionType === "players") return questions[0]?.text || translate("actionBoard.decision.choosePlayers");
    if (openDecision.decisionType === "equipment") return questions[0]?.text || translate("actionBoard.decision.chooseEquipment");
    return questions[0]?.text || openDecision.question;
  }
  const action = latestOpenAction(card);
  if (action) return action.text;
  if (card.completedAt) return translate("actionBoard.status.completed");
  const latestDecision = [...(card.decisions || [])].sort((a, b) => b.createdAt - a.createdAt)[0];
  if (latestDecision) return latestDecision.title?.trim() || (latestDecision.mode === "recorded"
    ? translate("actionBoard.decision.recordedChooseNext")
    : translate("actionBoard.decision.voteClosedChooseNext"));
  const latestAction = [...(card.actions || [])].sort((a, b) => b.createdAt - a.createdAt)[0];
  if (latestAction?.status === "done") return translate("actionBoard.action.completeChooseNext");
  return latestAction?.text || "";
}

function decisionTypeLabel(decision: TaskBoardVote) {
  if (decision.decisionType === "schedule" || decision.kind === "schedule") return translate("actionBoard.types.schedule");
  if (decision.decisionType === "players") return translate("actionBoard.types.players");
  if (decision.decisionType === "equipment") return translate("actionBoard.types.equipment");
  return decision.mode === "recorded" ? translate("actionBoard.types.decision") : translate("actionBoard.types.vote");
}
...
function currentNotifyTarget(card: TaskBoardCard): NotifyTarget | null {
  ...
  if (openDecision) {
    ...
    return {
      kind: "decision",
      id: openDecision.id,
      label: translate("actionBoard.types.decision"),
      text: openDecision.title?.trim() || (questions.length === 1 ? questions[0].text : openDecision.question || translate("actionBoard.types.decision")),
  ...
  if (openAction) {
    return {
      kind: "action",
      id: openAction.id,
      label: translate("actionBoard.types.todo"),
  ...
  if (latestEntry?.kind === "decision") {
    ...
    return {
      kind: "decision",
      id: decision.id,
      label: decision.kind === "schedule" || decision.decisionType === "schedule"
        ? translate("actionBoard.notification.scheduleResult")
        : translate("actionBoard.notification.decisionResult"),
      text: decision.title?.trim() || (questions.length === 1 ? questions[0].text : decision.question || translate("actionBoard.types.decision")),
  ...
  if (latestEntry?.kind === "action") {
    ...
    return {
      kind: "action",
      id: action.id,
      label: action.status === "done" ? translate("actionBoard.action.todoComplete") : translate("actionBoard.types.todo"),
      text: action.text,
  ...
  return {
    kind: "topic",
    label: translate("actionBoard.types.idea"),
    text: card.title,
...
function notificationSummary(notification?: TaskBoardNotificationState) {
  ...
  const channels = (notification.channels || []).map((channel) => channel === "email"
    ? translate("actionBoard.notification.channelEmail")
    : translate("actionBoard.notification.channelPhone")).join(" + ");
  const parts = [notification.sentByName
    ? translate("actionBoard.notification.notifiedBy", { name: notification.sentByName })
    : translate("actionBoard.notification.notified")];
  if (notification.sentAt) parts.push(formatTime(notification.sentAt));
  if (recipients) parts.push(translate("actionBoard.notification.organizerCount", { count: recipients }));
  if (channels) parts.push(channels);
  return parts.join(" · ");
}

function decisionHistoryMeta(decision: TaskBoardVote) {
  if (decision.mode === "recorded") return translate("actionBoard.status.recorded");
  const responses = voteTotal(decision);
  const questions = decisionQuestions(decision);
  const responseText = translate("actionBoard.vote.responseCount", { count: responses });
  if (decision.status === "open") return translate("actionBoard.vote.openWithResponses", { responses: responseText });
  if (questions.length > 1) return translate("actionBoard.vote.closedQuestionsResponses", { count: questions.length, responses: responseText });
  return translate("actionBoard.vote.closedWithResponses", { responses: responseText });
}
...
function decisionEvolutionResult(decision: TaskBoardVote) {
  if (decision.decisionType === "schedule" || decision.kind === "schedule") {
    if (decision.scheduleState === "finalized" || decision.finalizedTime) {
      const parts = [decision.finalizedTime ? scheduleLabel(decision.finalizedTime) : translate("actionBoard.schedule.timeConfirmed"), decision.finalizedLocation].filter(Boolean);
      return parts.join(" · ");
    }
    if (decision.scheduleState === "waiting-host") return translate("actionBoard.schedule.waitingForHost");
    if (decision.scheduleState === "setup") return decision.hostName
      ? translate("actionBoard.schedule.hostName", { name: decision.hostName })
      : translate("actionBoard.schedule.hostChosen");
    const responses = voteTotal(decision);
    return decision.status === "open"
      ? translate("actionBoard.vote.responseCount", { count: responses })
      : translate("actionBoard.schedule.closed");
  }
  if (decision.mode === "recorded") return decision.outcome || translate("actionBoard.decision.recorded");
...
function EmptyActionBoard({ onCreate }: { onCreate: () => void }) {
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col items-center px-4 py-10 text-center">
      ...
      <h3 className="mt-4 text-lg font-black text-white">{t("actionBoard.empty.title")}</h3>
      <p className="mt-1 max-w-md text-sm font-semibold leading-relaxed text-blue-100/90">{t("actionBoard.empty.description")}</p>
      <Button type="button" className="mt-5 h-11 rounded-2xl px-4 font-black text-white" onClick={onCreate}><Plus className="mr-1.5 h-4 w-4" />{t("actionBoard.card.add")}</Button>
    </div>
  );
}
...
export function TaskBoard({
  ...
  const [playerAnswerOptions, setPlayerAnswerOptions] = useState(defaultAnswerOptions);
  const [equipmentVoteMode, setEquipmentVoteMode] = useState<EquipmentVoteMode>("rate");
  const [equipmentAnswerOptions, setEquipmentAnswerOptions] = useState(defaultAnswerOptions);
  const [equipmentAnswersOpen, setEquipmentAnswersOpen] = useState(false);
  const [equipmentAnswerOptionsDraft, setEquipmentAnswerOptionsDraft] = useState(defaultAnswerOptions);
  ...
  useEffect(() => {
    ...
    return listenToTaskBoard(scopeId!, (snapshot) => {
      if (!snapshot.meta && snapshot.columns.length === 0 && !initialized && canEditBoard) {
        ...
        ]).catch((nextError) => setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.createBoard")));
      } else {
      ...
      setLoading(false);
      setError(nextError.message || t("actionBoard.error.loadBoard"));
    });
  ...
  const persistCard = async (card: TaskBoardCard) => {
    if (!canEditBoard) throw new Error(t("actionBoard.error.cannotEditShared"));
  ...
  const submitComment = async (card: TaskBoardCard) => {
    ...
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.addComment"));
    } finally {
  ...
  const saveEditedComment = async (card: TaskBoardCard, comment: TaskBoardComment) => {
    ...
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.editComment"));
    } finally {
      setCommentSavingCardId(null);
    }
  };
  ...
  const removeComment = async (card: TaskBoardCard, comment: TaskBoardComment) => {
    ...
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.deleteComment"));
    } finally {
      setCommentSavingCardId(null);
    }
  };
  ...
  const moveCardToStage = async (card: TaskBoardCard, stage: TopicStage) => {
    ...
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.moveCard"));
      return null;
    }
  };
  ...
  const sendNotification = async () => {
    ...
    if (!notifyEmailsToSend.length) {
      setNotifyError(t("actionBoard.error.chooseOrganizer"));
      return;
    }
    ...
    try {
      ...
      setNotifyError(nextError instanceof Error ? nextError.message : t("actionBoard.error.notifyOrganizers"));
    } finally {
  ...
  const createQuickTopic = async (stage: TopicStage) => {
    ...
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.addCard"));
    } finally {
      setSaving(false);
    }
  };
  ...
  const createTopic = async () => {
    ...
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.addTopic"));
  ...
  const saveEditedCard = async () => {
    ...
    catch (nextError) { setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.saveChanges")); }
    finally { setSaving(false); }
  };

  const removeCard = async () => {
    if (!canEditBoard || !editingCardId || !window.confirm(t("actionBoard.confirm.deleteTopic"))) return;
    ...
    try {
      ...
      setBoard(previous);
      setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.deleteTopic"));
    }
  };
  ...
  const startDecision = (card: TaskBoardCard, preset?: TaskBoardDecisionType) => {
    ...
    setPlayerSearch("");
    setPlayerAnswerOptions(defaultAnswerOptions());
    setEquipmentVoteMode("rate");
    setEquipmentAnswerOptions(defaultAnswerOptions());
    ...
    if (preset === "schedule") {
      ...
      setDecisionQuestion(t("actionBoard.decision.whoShouldBeSelected"));
      setDecisionMaxSelections("3");
  ...
  const createScheduleForHost = async (card: TaskBoardCard, host: TaskBoardPerson, requestDecisionId?: string) => {
    ...
    const scheduleDecision: TaskBoardVote = {
      ...
      question: t("actionBoard.schedule.setup"),
      hostName: host.name,
    ...
    const previous = (card.decisions || []).map((item) => requestDecisionId && item.id === requestDecisionId
      ? { ...item, status: "closed" as const, closedAt: now, closedByName: currentActor.name, outcome: t("actionBoard.schedule.hostedBy", { name: host.name }) }
      : item);
    const next: TaskBoardCard = {
      ...card,
    ...
    try {
      if (online && scopeId && requestDecisionId) {
        ...
        if (!claimed) {
          setError(t("actionBoard.error.hostAlreadyTaken"));
          return;
        }
      ...
      setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.startSchedule"));
    } finally { setSaving(false); }
  };
  ...
  const requestScheduleHost = async (mode: "person" | "group") => {
    ...
    const requestDecision: TaskBoardVote = {
      ...
      question: t("actionBoard.schedule.whoWillHost"),
      ...
      if (online && scopeId && recipients.length) {
        try {
          await sendActionBoardNotification({
            ...
            push: false,
            message: mode === "person"
              ? t("actionBoard.schedule.notificationAskedYou", { name: currentActor.name })
              : t("actionBoard.schedule.notificationLookingForHost", { name: currentActor.name }),
          });
        } catch { setError(t("actionBoard.error.hostRequestEmail")); }
      }
    } catch (nextError) {
      setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.requestHost"));
    } finally { setSaving(false); }
  };
  ...
  const declineScheduleHost = async (card: TaskBoardCard, requestDecision: TaskBoardVote) => {
    ...
    const nextDecisions = (card.decisions || []).map((item) => item.id === requestDecision.id ? {
      ...
      outcome: t("actionBoard.schedule.declinedHosting", { name: currentActor.name }),
      closedAt: now,
    ...
    catch (nextError) { setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.declineHost")); }
    finally { setSaving(false); }
  };
  ...
  const finalizeSchedule = async () => {
    ...
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.finalizeSchedule"));
    } finally { setSaving(false); }
  };
  ...
  const addDecision = async () => {
    ...
    if (decisionMode === "recorded") {
      ...
      if (decisionStep === "schedule") {
        ...
        if (values.length > 1) {
          ...
          questions.push({
            id: id("question"),
            text: t("actionBoard.schedule.whenCanYouMakeIt"),
            kind: "multi-select",
            scheduleRole: "time",
            options: [
              ...labels.map((label) => ({ id: id("option"), label, count: 0 })),
              { id: id("option"), label: t("actionBoard.schedule.noneAvailable"), count: 0 },
            ],
        ...
        if (locations.length > 1) {
          questions.push({
            id: id("question"),
            text: t("actionBoard.schedule.whereMeet"),
            kind: "choose-one",
    ...
    catch (nextError) { setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.addDecision")); }
    finally { setSaving(false); }
  };
  ...
  const closeVote = async (card: TaskBoardCard, decision: TaskBoardVote) => {
    ...
    catch (nextError) { setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.closeVote")); }
    finally { setSaving(false); }
  };
  ...
  const saveOutcome = async () => {
    ...
    catch (nextError) { setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.recordOutcome")); }
    finally { setSaving(false); }
  };
  ...
  const addLink = async () => {
    ...
    catch (nextError) { setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.addLink")); }
    finally { setSaving(false); }
  };
  ...
  const removeLink = async (card: TaskBoardCard, linkId: string) => {
    ...
    try { await persistCard(next); }
    catch (nextError) { setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.removeLink")); }
  };
  ...
  const fileSizeText = (size?: number) => {
    ...
    if (!size || size <= 0) return t("actionBoard.files.file");
    if (size < 1024) return `${formatNumber(locale, size)} B`;
    if (size < 1024 * 1024) {
      return `${formatNumber(locale, Math.max(1, Math.round(size / 1024)))} KB`;
    }
    return `${formatNumber(locale, size / (1024 * 1024), { minimumFractionDigits: 1, maximumFractionDigits: 1 })} MB`;
  };

  const openAddFile = (card: TaskBoardCard) => {
    if (!canEditClubResources || !isSharedRoster || !scopeId || !user?.email) {
      setError(t("actionBoard.error.signInForFiles"));
      return;
    }
    ...
    if (
      ...
      setError(
        t("actionBoard.error.fileLimit", { count: STRIPES_FILE_MAX_CARD_ATTACHMENTS }),
      );
      return;
    }
  ...
  const handleFileSelected = async (
    ...
    try {
      ...
      setError(
        nextError instanceof Error
          ? nextError.message
          : t("actionBoard.error.uploadFile"),
      );
    } finally {
      fileUploadTargetCardIdRef.current = null;
  ...
  const openCardFile = async (resource: ClubResource) => {
    ...
    if (popup) {
      popup.opener = null;
      popup.document.title = t("actionBoard.files.openingWindowTitle");
    }
  ...
  const removeCardFile = async (resource: ClubResource) => {
    ...
    if (
      typeof window !== "undefined"
      && !window.confirm(
        t("actionBoard.confirm.removeFile", { name: resource.name }),
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setError(
        nextError instanceof Error
          ? nextError.message
          : t("actionBoard.error.removeFile"),
      );
    } finally {
  ...
  const addAction = async () => {
    ...
    catch (nextError) { setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.saveTodo")); }
    finally { setSaving(false); }
  };
  ...
  const updateAction = async (
    ...
    try { await persistCard(next); }
    catch (nextError) { setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.updateAction")); }
  };
  ...
  const completeActionRound = async (card: TaskBoardCard, action?: TaskBoardActionItem) => {
    ...
    setError("");
    try { await persistCard(next); }
    catch (nextError) { setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.completeAction")); }
    finally { setSaving(false); }
  };
  ...
  const toggleVoteOption = (question: TaskBoardDecisionQuestion, optionId: string) => {
    setSelectedVoteAnswers((current) => {
      ...
      // Stripes-generated schedule-time questions append the exclusive none option last.
      // Its persisted label is presentation data and must never control voting behavior.
      const noneOption = question.scheduleRole === "time" && question.options.length >= 3
  ...
  const submitVote = async () => {
    ...
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.recordVote"));
  ...
  const saveBoardNameValue = async (value: string) => {
    ...
    try {
      ...
      setError(nextError instanceof Error ? nextError.message : t("actionBoard.error.renameBoard"));
    }
  };
  ...
  const renderPeoplePicker = (
    ...
    label = t("actionBoard.people.people"),
    excludeNames: string[] = [],
  ) => (
    <div>
      <Label>{label}</Label>
      <div className="mt-1.5 flex flex-wrap gap-1.5">
        ...
        {!availablePeople.length && <span className="text-[11px] font-semibold text-slate-400">{t("actionBoard.people.noOtherOrganizers")}</span>}
      </div>
    </div>
  );
  ...
  const renderVoterScope = () => (
    <div>
      <Label>{t("actionBoard.vote.whoVotes")}</Label>
      <div className="mt-1.5 grid w-full min-w-0 grid-cols-2 gap-2">
        ...
        >
          {t("actionBoard.people.allOrganizers")}
        </button>
        <button
          type="button"
        ...
        >
          {t("actionBoard.people.choose")}
        </button>
      </div>
      {decisionVoterMode === "selected" && (
        <div className="mt-2">{renderPeoplePicker(decisionPeopleKeys, setDecisionPeopleKeys, t("actionBoard.people.chooseOrganizers"))}</div>
      )}
    </div>
  );
  ...
  const renderDecision = (
    ...
    const heading = decision.title?.trim()
      || (decision.decisionType === "players"
        ? t("actionBoard.types.players")
        : decision.decisionType === "equipment"
          ? t("actionBoard.types.equipment")
          : questions.length === 1 ? questions[0].text : t("actionBoard.types.vote"));
    ...
    const results = (
      <div className="space-y-3">
        {decision.decisionType === "players" ? (
          <div className="overflow-hidden rounded-xl bg-white ring-1 ring-slate-100">
            {questions.map((question, questionIndex) => <div key={question.id} className={`flex items-center gap-2 px-2.5 py-2 ${questionIndex ? "border-t border-slate-100" : ""}`}>
              ...
              <div className="flex shrink-0 flex-wrap justify-end gap-1">{question.options.map((option) => <span key={option.id} className="rounded-full bg-slate-50 px-1.5 py-0.5 text-[9px] font-medium text-slate-600 ring-1 ring-slate-200">{option.label} {formatNumber(getResolvedUiLocale(), option.count)}</span>)}</div>
            </div>)}
          </div>
        ) : questions.map((question, questionIndex) => (
          <div key={question.id} className={questions.length > 1 ? "rounded-xl bg-slate-50 p-2.5" : ""}>
            {questions.length > 1 && <div className="mb-2 text-xs font-semibold leading-relaxed text-[#102A43]">{t("actionBoard.decision.numberedQuestion", { number: formatNumber(getResolvedUiLocale(), questionIndex + 1), question: question.text })}</div>}
            {decision.decisionType === "equipment" && (question.itemQuantity !== undefined || question.itemPrice || question.itemUrl) && <div className="mb-2 flex flex-wrap items-center gap-2 text-[10px] font-normal text-slate-500">{question.itemQuantity !== undefined && <span>{t("actionBoard.equipment.quantity", { count: question.itemQuantity })}</span>}{question.itemPrice && <span>{question.itemPrice}</span>}{question.itemPrice && question.itemQuantity !== undefined && <span className="font-medium text-slate-600">{t("actionBoard.equipment.subtotalValue", { value: equipmentSubtotal(question.itemPrice, question.itemQuantity) })}</span>}{question.itemUrl && <a href={question.itemUrl} target="_blank" rel="noreferrer" className="font-medium text-sky-700 underline">{t("actionBoard.equipment.openProductLink")}</a>}</div>}
                ...
                return (
                  <div key={option.id} className="rounded-xl bg-white px-2.5 py-2 ring-1 ring-slate-100">
                    <div className="flex items-center gap-2 text-xs font-medium text-slate-600">
                      <span className="min-w-0 flex-1 break-words">{option.label}{option.quantity !== undefined ? <span className="ml-1.5 text-[10px] font-normal text-slate-400">× {formatNumber(getResolvedUiLocale(), option.quantity)}</span> : null}{option.price ? <span className="ml-1.5 text-[10px] font-normal text-slate-400">{option.price}</span> : null}{option.price && option.quantity !== undefined ? <span className="ml-1.5 text-[10px] font-medium text-slate-500">{t("actionBoard.equipment.subtotalLowerValue", { value: equipmentSubtotal(option.price, option.quantity) })}</span> : null}</span>
                      <span className="shrink-0 rounded-full bg-slate-50 px-2 py-0.5 font-medium text-[#102A43] ring-1 ring-slate-200">{formatNumber(getResolvedUiLocale(), option.count)}</span>
                      {question.kind !== "multi-select" && <span className="w-8 text-right text-[10px] text-slate-400">{formatPercent(getResolvedUiLocale(), percent / 100, { maximumFractionDigits: 0 })}</span>}
                    </div>
                    {option.url && <a href={option.url} target="_blank" rel="noreferrer" className="mt-1 inline-block text-[10px] font-medium text-sky-700 underline">{t("actionBoard.equipment.openProductLink")}</a>}
        ...
        {decision.outcome && <button type="button" className="w-full rounded-xl bg-emerald-50 px-3 py-2 text-left text-[11px] font-bold text-emerald-800" onClick={() => openOutcome(card, decision)}><span className="font-semibold">{t("actionBoard.note.labelColon")}</span> {decision.outcome}</button>}
        {!decision.outcome && !open && <button type="button" className="px-1 text-[10px] font-medium text-slate-400 hover:text-slate-600" onClick={() => openOutcome(card, decision)}>{t("actionBoard.note.add")}</button>}
      </div>
    );
          ...
          <div className="rounded-2xl border border-amber-100 bg-amber-50/35 p-3">
            {scheduleState === "waiting-host" && <>
              <div className="mt-3 rounded-xl bg-white px-3 py-3 ring-1 ring-amber-100">
                <div className="text-xs font-semibold text-[#102A43]">{t("actionBoard.schedule.waitingForHost")}</div>
                <div className="mt-1 text-[11px] font-normal text-slate-500">{decision.hostRequestMode === "person"
                  ? t("actionBoard.schedule.organizerAskedToHost", { name: decision.requestedHostName || t("actionBoard.people.anOrganizer") })
                  : t("actionBoard.schedule.groupAskedToHost")}</div>
              </div>
              {(decision.hostRequestMode === "group" || requestedForMe) && <div className="mt-3 flex gap-2">
                <Button type="button" className="h-10 flex-1 rounded-xl bg-sky-700 text-xs font-semibold text-white hover:bg-sky-800" disabled={saving} onClick={() => void claimScheduleHost(card, decision)}>{t("actionBoard.schedule.illHost")}</Button>
                {requestedForMe && <button type="button" className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-xs font-medium text-slate-500" disabled={saving} onClick={() => void declineScheduleHost(card, decision)}>{t("actionBoard.schedule.decline")}</button>}
              </div>}
            </>}

            {scheduleState === "host-declined" && <>
              <div className="mt-3 rounded-xl bg-white px-3 py-3 ring-1 ring-amber-100">
                <div className="text-xs font-semibold text-[#102A43]">{t("actionBoard.schedule.requestDeclined")}</div>
                <div className="mt-1 text-[11px] font-normal text-slate-500">{t("actionBoard.schedule.requestDeclinedHelp")}</div>
              </div>
            </>}

            {scheduleState === "setup" && <>
              <div className="mt-3 rounded-xl bg-white px-3 py-3 ring-1 ring-sky-100">
                <div className="text-[10px] font-medium text-slate-400">{t("actionBoard.schedule.host")}</div>
                <div className="mt-0.5 text-sm font-semibold text-[#102A43]">{decision.hostName}</div>
              </div>
              {isHost ? <Button type="button" className="mt-3 h-10 w-full rounded-xl bg-sky-700 text-xs font-semibold text-white hover:bg-sky-800" onClick={() => openScheduleSetup(card, decision)}>{t("actionBoard.schedule.setup")}</Button> : <div className="mt-3 text-[11px] font-normal text-slate-500">{t("actionBoard.schedule.waitingForProposal", { name: decision.hostName || t("actionBoard.schedule.theHost") })}</div>}
            </>}

            {scheduleState === "collecting" && <>
              <div className="mt-2 flex flex-wrap gap-1.5 text-[10px] font-medium text-slate-500">
                {decision.hostName && <span className="rounded-full bg-white px-2 py-1 ring-1 ring-slate-200">{t("actionBoard.schedule.hostName", { name: decision.hostName })}</span>}
                {decision.participantNames?.length ? <span className="rounded-full bg-white px-2 py-1 ring-1 ring-slate-200">{t("actionBoard.schedule.participantCount", { count: decision.participantNames.length })}</span> : null}
              </div>
              {decision.scheduleTimeValues?.length === 1 && <div className="mt-3 rounded-xl bg-white px-3 py-2.5 ring-1 ring-slate-100"><div className="text-[10px] font-medium text-slate-400">{t("actionBoard.schedule.proposedTime")}</div><div className="mt-0.5 text-sm font-semibold text-[#102A43]">{scheduleLabel(decision.scheduleTimeValues[0])}</div></div>}
              {decision.scheduleLocationOptions?.length === 1 && <div className="mt-2 rounded-xl bg-white px-3 py-2.5 ring-1 ring-slate-100"><div className="text-[10px] font-medium text-slate-400">{t("actionBoard.schedule.proposedLocation")}</div><div className="mt-0.5 text-sm font-semibold text-[#102A43]">{decision.scheduleLocationOptions[0]}</div></div>}
              {questions.length > 0 && <div className="mt-3">{results}</div>}
              <div className="mt-3 flex flex-wrap items-center gap-2">
                {questions.length > 0 && <span className="mr-auto text-[10px] font-medium text-slate-500">{decision.eligibleCount
                  ? t("actionBoard.vote.responseProgress", { count: totalVoters, total: decision.eligibleCount })
                  : t("actionBoard.vote.respondedCount", { count: totalVoters })}</span>}
                {questions.length > 0 && canVote && <button type="button" className="rounded-xl bg-amber-600 px-3 py-2 text-xs font-semibold text-white" onClick={() => openVoteDialog(card, decision)}>{t("actionBoard.vote.respond")}</button>}
                {isHost && totalVoters === 0 && <button type="button" className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-600" onClick={() => openScheduleSetup(card, decision)}>{t("common.edit")}</button>}
                {isHost && <button type="button" className="rounded-xl bg-sky-700 px-3 py-2 text-xs font-semibold text-white" onClick={() => openFinalizeSchedule(card, decision)}>{t("actionBoard.schedule.finalizeShort")}</button>}
              </div>
            </>}

            {scheduleState === "finalized" && <>
              <div className="mt-3 rounded-2xl bg-white p-3 ring-1 ring-amber-100">
                <div className="text-[10px] font-medium uppercase tracking-wide text-slate-400">{t("actionBoard.schedule.confirmed")}</div>
                <div className="mt-1 text-base font-semibold text-[#102A43]">{decision.finalizedTime ? scheduleLabel(decision.finalizedTime) : t("actionBoard.schedule.timeConfirmed")}</div>
                ...
                {decision.hostName && <div className="mt-1 text-[11px] font-normal text-slate-500">{t("actionBoard.schedule.hostName", { name: decision.hostName })}</div>}
                {decision.meetingUrl && <a href={decision.meetingUrl} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-sky-700"><ExternalLink className="h-3.5 w-3.5" />{t("actionBoard.schedule.openMeetingLink")}</a>}
              </div>
              <div className="mt-3 flex flex-wrap gap-2">
                {calendarUrl && <a href={calendarUrl} target="_blank" rel="noreferrer" className="inline-flex h-10 items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700"><CalendarDays className="h-4 w-4" />{t("actionBoard.schedule.addToGoogleCalendar")}</a>}
                {online && currentNotifyTarget(card)?.notification?.status !== "sent" && <button type="button" className="inline-flex h-10 items-center gap-1.5 rounded-xl border border-amber-200 bg-white px-3 text-xs font-semibold text-amber-800" onClick={() => openNotify(card)}><Bell className="h-4 w-4" />{t("actionBoard.notification.notify")}</button>}
              </div>
            </>}
          </div>
        </div>
      ...
      return (
        <div key={entryKey} className="relative pl-8">
          ...
          <button type="button" className="w-full rounded-xl bg-white/80 px-3 py-2.5 text-left ring-1 ring-slate-200/80 transition hover:bg-white" onClick={() => toggleHistoryExpanded(entryKey)}>
            <div className="flex items-start gap-2">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1 text-[9px] font-semibold uppercase tracking-wide text-slate-400 lg:text-[10px]"><span>{decisionTypeLabel(decision)} · {decision.mode === "recorded" ? t("actionBoard.status.recordedLower") : decision.status === "open" ? t("actionBoard.status.openLower") : decision.status === "closed" ? t("actionBoard.status.closedLower") : decision.status}</span>{decision.notification?.status === "sent" && <span className="inline-flex items-center gap-0.5 text-emerald-600" title={notificationSummary(decision.notification)}><Bell className="h-2.5 w-2.5 fill-emerald-100" /><Check className="h-2.5 w-2.5" /></span>}</div>
                ...
                <div className="mt-1 text-[9px] font-bold text-slate-400 lg:text-[10px]">{decisionHistoryMeta(decision)} · {historyExpanded ? t("actionBoard.history.hideDetails") : open ? t("actionBoard.history.openDetails") : t("actionBoard.history.viewResult")}</div>
              </div>
          ...
          {historyExpanded && <div className="mt-2 rounded-xl border border-slate-200 bg-white p-3">
            {decision.hostName && <div className="mb-1 text-[10px] font-bold text-slate-500">{t("actionBoard.schedule.hostColon")} <span className="font-semibold text-slate-700">{decision.hostName}</span></div>}
            ...
            {decision.mode === "recorded" ? (
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className="mr-auto text-[10px] font-bold text-slate-500">{decision.eligibleCount
                  ? t("actionBoard.vote.responseProgress", { count: totalVoters, total: decision.eligibleCount })
                  : t("actionBoard.vote.respondedCount", { count: totalVoters })}</span>
                {canVote ? <button type="button" className="rounded-xl bg-amber-600 px-3 py-2 text-xs font-semibold text-white" onClick={() => openVoteDialog(card, decision)}>{decision.kind === "schedule" ? t("actionBoard.vote.availability") : t("actionBoard.types.vote")}</button> : <span className="text-[10px] font-medium text-slate-400">{t("actionBoard.vote.notAskingYou")}</span>}
                <button type="button" className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-500" onClick={() => void closeVote(card, decision)}>{t("common.close")}</button>
              </div>
            ) : results}
          </div>}
        </div>
      ...
      <div key={entryKey} className="mt-1">
        {decision.mode === "recorded" ? (
          ...
          <div className="flex flex-wrap items-center gap-2 border-y border-slate-100 py-2.5">
            <span className="mr-auto text-[11px] font-medium text-slate-500">{decision.eligibleCount
              ? t("actionBoard.vote.responseProgress", { count: totalVoters, total: decision.eligibleCount })
              : t("actionBoard.vote.respondedCount", { count: totalVoters })}</span>
            {canVote ? <button type="button" className="rounded-xl bg-amber-600 px-3 py-2 text-xs font-semibold text-white" onClick={() => openVoteDialog(card, decision)}>{t("actionBoard.types.vote")}</button> : null}
            <button type="button" className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-500" onClick={() => void closeVote(card, decision)}>{t("common.close")}</button>
          </div>
    ...
    return (
      <div key={entryKey} className="relative pl-8">
        ...
        <div className="rounded-2xl border border-sky-100 bg-sky-50/35 p-3">
          <div className="text-[9px] font-semibold uppercase tracking-wide text-sky-700">{t("actionBoard.action.todoDetails")}</div>
          ...
          {assignees.length > 0 ? <div className="mt-1 text-[10px] font-medium text-sky-800"><Users className="mr-1 inline h-3 w-3" />{t("actionBoard.people.assignedTo", { people: personSummary(assignees) })}</div> : <div className="mt-1 text-[10px] font-normal text-slate-400">{t("actionBoard.people.noAssignee")}</div>}
        </div>
      </div>
    );
  };
  ...
  const renderEvolution = (card: TaskBoardCard) => {
    const entries = timelineEntries(card);
    const stage = stageForCard(card);
    const stageLabel = stage === "deciding" ? t("actionBoard.workflow.decide") : stage === "action" ? t("actionBoard.workflow.todo") : stage === "done" ? t("actionBoard.workflow.done") : t("actionBoard.workflow.ideas");
        ...
        <div className="relative space-y-4">
          <div className="relative pl-10 lg:pl-12">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white px-3 py-3">
              <div className="text-[10px] font-medium uppercase tracking-wide text-slate-400">{t("actionBoard.history.createdAt", { time: formatTime(card.createdAt) })}</div>
              <div className="mt-1 text-sm font-semibold text-[#102A43]">{card.title}</div>
              <div className="mt-1 text-[11px] font-normal text-slate-500">{card.createdByName ? t("actionBoard.history.addedBy", { name: card.createdByName }) : t("actionBoard.history.cardCreated")}</div>
            </div>
          </div>
          {entries.map((entry) => entry.kind === "decision" ? (
            <div key={entry.key} className="relative pl-10 lg:pl-12">
              ...
              <div className="rounded-2xl border border-amber-100 bg-amber-50/35 px-3 py-3">
                ...
                <div className="mt-1 text-sm font-semibold leading-snug text-[#102A43]">{entry.decision.title?.trim() || entry.decision.question || t("actionBoard.types.decision")}</div>
                <div className="mt-1.5 text-[11px] font-normal leading-relaxed text-slate-600">{decisionEvolutionResult(entry.decision)}</div>
                {entry.decision.hostName && <div className="mt-1 text-[10px] font-medium text-slate-500">{t("actionBoard.schedule.hostName", { name: entry.decision.hostName })}</div>}
              </div>
            </div>
          ) : (
            <div key={entry.key} className="relative pl-10 lg:pl-12">
              ...
              <div className="rounded-2xl border border-sky-100 bg-sky-50/35 px-3 py-3">
                <div className="flex flex-wrap items-center gap-1.5 text-[10px] font-medium uppercase tracking-wide text-sky-700"><span>{t("actionBoard.types.todo")}</span><span className="text-sky-400">·</span><span>{formatTime(entry.createdAt)}</span></div>
                ...
                <div className="mt-1.5 text-[11px] font-normal text-slate-600">{entry.action.status === "done"
                  ? entry.action.completedByName ? t("actionBoard.history.completedBy", { name: entry.action.completedByName }) : t("actionBoard.status.completed")
                  : t("actionBoard.status.open")}</div>
              </div>
            </div>
          ))}
          <div className="relative pl-10 lg:pl-12">
            ...
            <div className={`rounded-2xl px-3 py-3 ring-1 ${stageTone}`}><div className="text-[10px] font-medium uppercase tracking-wide opacity-70">{t("actionBoard.history.now")}</div><div className="mt-1 text-sm font-semibold">{stageLabel}</div></div>
          </div>
        </div>
      </div>
    );
    ...
    if (stage === "deciding") {
      ...
      return (
        <div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50/55 p-3">
          <div className="flex items-center gap-2 text-xs font-semibold text-amber-900"><CheckCircle2 className="h-4 w-4" />{t("actionBoard.decision.complete")}</div>
          <div className="mt-1 text-[10px] font-medium leading-relaxed text-amber-800/75">{repeatLabel ? t("actionBoard.decision.continueRoundHelp") : t("actionBoard.decision.finishedHelp")}</div>
          <div className={`mt-3 grid gap-2 ${repeatLabel ? "grid-cols-2" : "grid-cols-1"}`}>
            ...
            <button type="button" className="h-10 rounded-xl bg-[#102A43] px-3 text-[11px] font-semibold text-white" onClick={() => openMoveCard(card)}>{t("actionBoard.card.move")}</button>
          </div>
        </div>
      );
    }
    ...
    return (
      <section className="mt-5 border-t border-slate-200 pt-4">
        <div className="mb-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-sm font-semibold text-[#102A43]"><MessageCircle className="h-4 w-4 text-slate-500" />{t("actionBoard.comments.title")}{comments.length ? <span className="text-xs font-medium text-slate-400">· {formatNumber(getResolvedUiLocale(), comments.length)}</span> : null}</div>
          <div className="text-[10px] font-normal text-slate-400">{t("actionBoard.comments.help")}</div>
        </div>

        <div className="space-y-3">
          {comments.length === 0 && <div className="rounded-2xl bg-slate-50 px-3 py-4 text-center text-xs font-normal text-slate-400">{t("actionBoard.comments.empty")}</div>}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
                      ...
                      <span className="text-[10px] font-normal text-slate-400">{comment.updatedAt ? t("actionBoard.comments.editedAt", { time: formatTime(comment.createdAt) }) : formatTime(comment.createdAt)}</span>
                    </div>
                  </div>
                  {canEditBoard && mine && !editing && <div className="flex shrink-0 items-center gap-0.5 opacity-70 transition group-hover:opacity-100">
                    <button type="button" className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 hover:bg-white hover:text-slate-600" onClick={() => beginEditComment(comment)} aria-label={t("actionBoard.comments.edit")}><Pencil className="h-3.5 w-3.5" /></button>
                    <button type="button" className="flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 hover:bg-white hover:text-red-600" onClick={() => void removeComment(card, comment)} aria-label={t("actionBoard.comments.delete")}><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>}
                </div>
                {editing ? (
                  <div className="mt-2">
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <Button type="button" className="h-8 rounded-xl px-3 text-xs font-semibold text-white" style={{ backgroundColor: accent }} disabled={!editingCommentText.trim() || busy} onClick={() => void saveEditedComment(card, comment)}>{t("common.save")}</Button>
                      <button type="button" className="h-8 rounded-xl px-2 text-xs font-medium text-slate-400 hover:bg-white hover:text-slate-600" onClick={() => { setEditingCommentId(null); setEditingCommentText(""); }}>{t("common.cancel")}</button>
                    </div>
                  </div>
        ...
        {canEditBoard && <div className={`${detail ? "sticky bottom-0 z-[5] -mx-4 mt-4 border-t border-slate-200 bg-white/95 px-4 pb-[max(.75rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur" : "mt-4"}`}>
          <div className="flex items-end gap-2">
            <Textarea
              ...
              maxLength={3000}
              rows={2}
              placeholder={t("actionBoard.comments.placeholder")}
              aria-label={t("actionBoard.comments.commentOn", { title: card.title })}
            ...
            />
            <button type="button" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#102A43] text-white shadow-sm disabled:cursor-not-allowed disabled:opacity-40" disabled={!draft.trim() || busy} onClick={() => void submitComment(card)} aria-label={t("actionBoard.comments.post")}><Send className="h-4 w-4" /></button>
          </div>
        </div>}
      </section>
    );
    ...
    return (
      ...
      >
          ...
          <div className="flex items-start gap-2">
            <button type="button" className="min-w-0 flex-1 text-left" onClick={() => openCardDetail(card)}>
              ...
              {(card.category || card.dueDate || displayPeople.length > 0 || (card.comments?.length || 0) > 0 || decisionComplete || needsYourVote || openDecision) && <div className="mt-1.5 flex min-w-0 items-center gap-1 overflow-hidden whitespace-nowrap">
                {card.category && <span className={`max-w-[28%] shrink truncate rounded-full px-1.5 py-0.5 text-[9px] font-medium ring-1 lg:text-[11px] ${tagTone(card.category)}`}>{tagLabel(card.category)}</span>}
                ...
                {displayPeople.length > 0 && <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-medium text-slate-600 lg:min-w-0 lg:max-w-[30%] lg:shrink lg:text-[11px]" title={t("actionBoard.people.assignedTo", { people: personSummary(displayPeople) })}><Users className="h-3 w-3 shrink-0" /><span className="lg:hidden">{formatNumber(getResolvedUiLocale(), displayPeople.length)}</span><span className="hidden truncate lg:inline">{personSummary(displayPeople)}</span></span>}
                {(card.comments?.length || 0) > 0 && <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-medium text-slate-600 lg:text-[11px]" title={t("actionBoard.comments.count", { count: card.comments!.length })}><MessageCircle className="h-3 w-3" />{formatNumber(getResolvedUiLocale(), card.comments!.length)}</span>}
                {needsYourVote && <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-amber-100 px-1.5 py-0.5 text-[9px] font-semibold text-amber-900 ring-1 ring-amber-200 lg:text-[11px]"><span className="h-1.5 w-1.5 rounded-full bg-amber-600" />{t("actionBoard.vote.yourVote")}</span>}
                {decisionComplete && <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-amber-50 px-1.5 py-0.5 text-[9px] font-medium text-amber-800 ring-1 ring-amber-200 lg:text-[11px]"><Check className="h-3 w-3" />{t("actionBoard.status.complete")}</span>}
                {openDecision && <span className="ml-auto hidden shrink-0 text-[10px] font-medium text-slate-400 lg:inline">{openDecision.eligibleCount
                  ? t("actionBoard.vote.respondedRatio", { count: voteTotal(openDecision), total: openDecision.eligibleCount })
                  : t("actionBoard.vote.respondedCount", { count: voteTotal(openDecision) })}</span>}
              </div>}
            </button>
            <div className="flex shrink-0 flex-col items-center gap-0.5">
              {canNotifyBoard && cardNotifyTarget && (
                cardNotificationSent ? (
                  <span className="relative rounded-full bg-emerald-50 p-1.5 text-emerald-700" title={notificationSummary(cardNotification)} aria-label={notificationSummary(cardNotification) || t("actionBoard.notification.notified")}>
                  ...
                  <button type="button" className={`rounded-full p-1.5 ${cardNotificationQueued ? "cursor-wait bg-amber-50 text-amber-600" : "text-slate-400 hover:bg-amber-50 hover:text-amber-700"}`} onClick={() => openNotify(card)} disabled={cardNotificationQueued} aria-label={cardNotificationQueued ? t("actionBoard.notification.sending") : t("actionBoard.notification.notifyAbout", { target: cardNotifyTarget.label.toLowerCase() })} title={cardNotificationQueued ? t("actionBoard.notification.sendingEllipsis") : t("actionBoard.notification.notifyOrganizers")}><Bell className={`h-3.5 w-3.5 ${cardNotificationQueued ? "animate-pulse" : ""}`} /></button>
                )
              )}
              {canEditBoard && <button type="button" className="rounded-full px-1.5 py-1 text-[9px] font-semibold text-slate-400 hover:bg-slate-50 hover:text-slate-600 lg:text-[11px]" onClick={() => openMoveCard(card)} aria-label={t("actionBoard.card.moveNamed", { title: card.title })}>{t("actionBoard.card.moveShort")}</button>}
              <button type="button" className="hidden rounded-full p-1.5 text-slate-400 hover:bg-slate-50 lg:block" onClick={() => toggleExpanded(card.id)} aria-label={expanded ? t("actionBoard.card.collapse") : t("actionBoard.card.expand")}>{expanded ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}</button>
            </div>
          </div>
        </div>}
        ...
        {expanded && <div className={detail ? "bg-white px-4 pb-24 pt-5" : "border-t border-slate-100 bg-slate-50/35 px-3 pb-3 pt-3"}>
          ...
          {card.note?.trim() && <div className="mb-3 rounded-2xl bg-slate-50 px-3 py-3"><div className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{t("actionBoard.note.label")}</div><p className="mt-1 whitespace-pre-wrap break-words text-sm font-normal leading-relaxed text-slate-700">{card.note.trim()}</p></div>}
          ...
          {fileUploadingCardId === card.id && (
            <div className="mb-3 flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-xs font-medium text-slate-600">{t("actionBoard.files.uploading")}</div>
                <div className="text-[10px] font-normal text-slate-400">{t("actionBoard.files.keepOpen")}</div>
              </div>
            </div>
          )}
                ...
                >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="block truncate text-[10px] font-normal text-slate-400">
                        {fileOpeningResourceId === resource.id
                          ? t("actionBoard.files.opening")
                          : `${fileSizeText(resource.size)} · ${resource.createdByName}${resource.createdAt ? ` · ${formatDateTime(getResolvedUiLocale(), new Date(resource.createdAt))}` : ""}`}
                      </span>
                    </span>
                  ...
                  {canEditClubResources && stage !== "done" && (
                    <button
                      ...
                      aria-label={t("actionBoard.common.removeNamed", { name: resource.name })}
                    >
          ...
          {(decisions.length > 0 || actions.length > 0) && <div className="mb-3 flex justify-end"><button type="button" className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 text-[11px] font-medium text-slate-600 hover:bg-slate-50" onClick={() => setEvolutionCardId(card.id)}><History className="h-3.5 w-3.5" />{t("actionBoard.evolution.stepCount", { count: evolutionStepCount(card) })}</button></div>}
          {renderCurrentChapter(card)}
          {renderStageTools(card, stage)}
          {canEditBoard && !(stage === "deciding" && !openDecision && Boolean(currentDecision)) && <button type="button" className="mt-3 inline-flex h-10 w-full items-center justify-center rounded-2xl border border-slate-200 bg-white text-xs font-semibold text-slate-600" onClick={() => openMoveCard(card)}>{t("actionBoard.card.move")}</button>}
  ...
  const boardColumn = (stage: TopicStage, title: string, Icon: React.ComponentType<{ className?: string }>, mobile = false) => {
    ...
    const addPlaceholder = stage === "ideas"
      ? t("actionBoard.card.addIdeaPlaceholder")
      : stage === "deciding"
        ? t("actionBoard.card.decisionPlaceholder")
        : t("actionBoard.card.todoPlaceholder");
    ...
    return (
      ...
      >
        <div className={`mb-2 flex items-center gap-1.5 px-1 text-[15px] font-semibold lg:text-[18px] ${stage === "deciding" ? "text-amber-800" : stage === "action" ? "text-sky-800" : stage === "done" ? "text-slate-500" : "text-slate-700"}`}><Icon className="h-4 w-4 lg:h-5 lg:w-5" /><span>{title}</span><span className="ml-0.5 rounded-full bg-white/80 px-1.5 py-0.5 text-[9px] lg:px-2 lg:text-[11px] font-semibold text-slate-500 ring-1 ring-slate-200/70">{formatNumber(getResolvedUiLocale(), cardsByStage[stage].length)}</span></div>
        <div className="space-y-2 lg:space-y-2.5">
          {cardsByStage[stage].map((card) => renderCard(card, true))}
          {cardsByStage[stage].length === 0 && <div className="rounded-2xl border border-dashed border-slate-200 bg-white/40 px-2 py-5 text-center text-[10px] font-medium text-slate-400 lg:text-[13px]">{stage === "ideas" ? t("actionBoard.column.emptyIdeas") : stage === "deciding" ? t("actionBoard.column.emptyDecisions") : stage === "action" ? t("actionBoard.column.emptyTodos") : t("actionBoard.column.emptyDone")}</div>}
          ...
          {canAdd && (quickAddStage === stage ? (
            <form className="rounded-2xl border border-slate-200 bg-white p-2 shadow-sm" onSubmit={(event) => { event.preventDefault(); void createQuickTopic(stage); }}>
              <Input
                ...
                aria-label={t("actionBoard.card.addToColumn", { column: title })}
              ...
              />
              <div className="mt-2 flex items-center gap-2">
                <Button type="submit" className="h-8 rounded-xl px-3 text-xs font-semibold text-white" style={{ backgroundColor: accent }} disabled={!quickAddTitle.trim() || saving}>{saving ? t("actionBoard.card.adding") : t("common.add")}</Button>
                <button type="button" className="h-8 rounded-xl px-2 text-xs font-medium text-slate-400 hover:bg-slate-50 hover:text-slate-600" onClick={() => { setQuickAddStage(null); setQuickAddTitle(""); }}>{t("common.cancel")}</button>
              </div>
            </form>
          ) : (
            <button type="button" className="flex h-9 w-full items-center gap-1.5 rounded-xl px-2 text-left text-xs font-medium text-slate-500 transition hover:bg-white/80 hover:text-slate-700 lg:h-10 lg:text-[13px]" onClick={() => focusQuickAdd(stage)}><Plus className="h-3.5 w-3.5" />{t("actionBoard.card.add")}</button>
          ))}
        </div>
      </section>
    );
    ...
    <>
        ...
        <div className="stripes-type-ui flex items-start justify-between gap-3">
          <button type="button" className="flex min-w-0 flex-1 items-center gap-2.5 text-left active:scale-[0.99]" onClick={openBoard}>
            ...
            <span className="min-w-0">
              <span className="block text-[17px] font-black leading-tight text-white lg:text-[22px]">{t("actionBoard.title")}</span>
              <span className="mt-0.5 block truncate text-[10px] font-bold text-blue-100/85 lg:text-[13px]">
                {customBoardName || (online ? t("actionBoard.summary.shared") : isSharedRoster ? t("actionBoard.summary.authority", { authority: sharedAuthorityMessage || t("actionBoard.access.unavailable") }) : t("actionBoard.summary.private"))}
              </span>
            </span>
          </button>
          <button type="button" className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white/85 shadow-sm ring-1 ring-white/80 active:scale-[0.98] lg:hidden" style={{ color: accent }} onClick={openBoard} aria-label={t("actionBoard.open")}>
            <ChevronRight className="h-4 w-4" />
          </button>
          <Button type="button" className="hidden h-9 shrink-0 rounded-2xl px-3 text-xs font-black text-white lg:inline-flex lg:text-sm" style={{ backgroundColor: accent }} onClick={openBoard}>{t("common.open")}</Button>
        </div>
        <div className="mt-3 hidden items-center gap-2 text-[10px] font-black text-blue-100/90 lg:flex lg:text-[13px]">
          {hasNewActivity && <span className="inline-flex items-center gap-1 rounded-full bg-amber-50 px-2.5 py-1 text-amber-700"><span className="h-1.5 w-1.5 rounded-full bg-amber-500" />{t("actionBoard.activity.new")}</span>}
          <span className="rounded-full bg-white/15 px-2.5 py-1 text-white ring-1 ring-white/20">{online ? (canEditBoard ? t("actionBoard.access.shared") : t("actionBoard.access.sharedViewOnly")) : isSharedRoster ? (sharedAuthorityMessage || t("actionBoard.access.unavailable")) : t("actionBoard.access.private")}</span>
        </div>
        {latestActivity && <div className="mt-2 hidden truncate text-[10px] font-bold text-blue-100/80 lg:block lg:text-[13px]">{t("actionBoard.activity.last", { title: latestActivity.card.title, activity: activityText(latestActivity.activity) })}</div>}
      </section>
      ...
      <Dialog open={boardOpen} onOpenChange={(open) => { setBoardOpen(open); if (!open) setActiveCardId(null); }}>
        <DialogContent className="stripes-type-ui fixed inset-0 flex h-[100dvh] max-h-none w-full max-w-none translate-x-0 translate-y-0 flex-col gap-0 overflow-hidden rounded-none border-0 bg-white p-0 shadow-none [&>button]:text-white [&>button]:opacity-80 sm:inset-3 sm:h-[calc(100dvh-1.5rem)] sm:w-auto sm:rounded-[2rem] sm:border sm:border-white/70 sm:shadow-2xl lg:inset-6 lg:h-[calc(100dvh-3rem)] lg:rounded-[2rem]">
            ...
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <div className="flex min-w-0 items-center gap-2">
                  <DialogTitle className="shrink-0 text-[17px] font-semibold text-white lg:text-[22px] lg:leading-tight">{t("actionBoard.title")}</DialogTitle>
                ...
                <p className="mt-0.5 text-[10px] font-medium text-blue-100/80 lg:text-[12px]"><span className="lg:hidden"><Trans i18nKey="actionBoard.instructions.mobile" components={{ move: <span className="font-semibold text-white" /> }} /></span><span className="hidden lg:inline"><Trans i18nKey="actionBoard.instructions.desktop" components={{ move: <span className="font-semibold text-white" /> }} /></span></p>
              </div>
              {canEditBoard && <Button type="button" variant="outline" className="h-8 w-8 shrink-0 rounded-xl border-white/30 bg-white/90 p-0 text-[#102A43] lg:h-9 lg:w-9" onClick={() => { setNotifyError(""); setBoardNameDraft(customBoardName || ""); setBoardSettingsOpen(true); }} aria-label={t("actionBoard.settings.title")} title={t("actionBoard.settings.title")}><Settings className="h-4 w-4" /></Button>}
            </div>
          </DialogHeader>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto" style={{ backgroundColor: background }}>
            {loading ? <div className="p-8 text-center text-sm font-semibold text-blue-100">{t("actionBoard.loading")}</div> : (
              <>
                <div className="flex snap-x snap-mandatory gap-3 overflow-x-auto overscroll-x-contain px-[9vw] py-3 pb-20 scroll-smooth lg:hidden" style={{ WebkitOverflowScrolling: "touch", touchAction: "pan-x pan-y" }}>
                  {boardColumn("ideas", t("actionBoard.workflow.ideas"), Lightbulb, true)}
                  {boardColumn("deciding", t("actionBoard.workflow.decide"), Gavel, true)}
                  {boardColumn("action", t("actionBoard.workflow.todo"), ClipboardList, true)}
                  {boardColumn("done", t("actionBoard.workflow.done"), Check, true)}
                </div>
                <div className="hidden w-full grid-cols-[repeat(4,minmax(14rem,1fr))] gap-3 overflow-x-auto p-4 pb-16 lg:grid xl:grid-cols-[repeat(4,minmax(15rem,1fr))] xl:gap-4 xl:p-5">
                  {boardColumn("ideas", t("actionBoard.workflow.ideas"), Lightbulb)}
                  {boardColumn("deciding", t("actionBoard.workflow.decide"), Gavel)}
                  {boardColumn("action", t("actionBoard.workflow.todo"), ClipboardList)}
                  {boardColumn("done", t("actionBoard.workflow.done"), Check)}
                </div>
              </>
            )}
          </div>
          ...
          {activeCardId && (() => {
            ...
            const stageLabel = activeStage === "deciding" ? t("actionBoard.workflow.decide") : activeStage === "action" ? t("actionBoard.workflow.todo") : activeStage === "done" ? t("actionBoard.workflow.done") : t("actionBoard.types.idea");
            return (
              <div className="absolute inset-0 z-40 flex flex-col bg-white lg:hidden">
                <div className="sticky top-0 z-10 flex shrink-0 items-center gap-2 border-b border-slate-200 bg-white/95 px-2 py-2.5 backdrop-blur">
                  <button type="button" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl text-[#102A43] active:bg-slate-100" onClick={closeCardDetail} aria-label={t("actionBoard.backToBoard")}>
                    <ArrowLeft className="h-5 w-5" />
                  </button>
                  <div className="min-w-0 flex-1">
                    <div className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">{stageLabel}</div>
                  </div>
                  {canEditBoard && <button type="button" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl text-slate-500 active:bg-slate-100" onClick={() => openEditCard(activeCard)} aria-label={t("actionBoard.card.edit")}>
              ...
              <div className="absolute inset-y-0 right-0 z-40 hidden w-[min(34rem,46vw)] flex-col border-l border-slate-200 bg-white shadow-[-18px_0_40px_rgba(15,23,42,0.10)] lg:flex">
                <div className="flex shrink-0 items-center gap-2 border-b border-slate-200 bg-white/95 px-3 py-3 backdrop-blur">
                  <button
                    ...
                    aria-label={t("actionBoard.card.closeDetails")}
                  >
                    <ArrowLeft className="h-5 w-5" />
                  </button>
                  <div className="min-w-0 flex-1">
                  ...
                  {canEditBoard && <button
                    ...
                    aria-label={t("actionBoard.card.edit")}
                  >
                    <Pencil className="h-4 w-4" />
                  </button>}
                </div>
      ...
      <Dialog open={Boolean(evolutionCardId)} onOpenChange={(open) => { if (!open) setEvolutionCardId(null); }}>
        <StripesWorkspaceContent>
          <DialogHeader className="shrink-0 border-b border-slate-200 bg-white px-3 py-3 pr-12 text-left lg:px-5 lg:py-4">
            <div className="flex items-center gap-2"><button type="button" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl text-[#102A43] hover:bg-slate-100" onClick={() => setEvolutionCardId(null)} aria-label={t("actionBoard.backToCard")}><ArrowLeft className="h-5 w-5" /></button><div className="min-w-0"><DialogTitle className="flex items-center gap-2 text-base font-semibold text-[#102A43] lg:text-xl"><History className="h-4 w-4 text-slate-500 lg:h-5 lg:w-5" />{t("actionBoard.evolution.title")}</DialogTitle>{evolutionCard && <p className="mt-0.5 truncate text-[11px] font-normal text-slate-500 lg:text-sm">{evolutionCard.title}</p>}</div></div>
          </DialogHeader>
          <div className="min-h-0 flex-1 overflow-y-auto bg-slate-50/50">{evolutionCard ? renderEvolution(evolutionCard) : <div className="p-8 text-center text-sm text-slate-500">{t("actionBoard.evolution.unavailable")}</div>}</div>
        </StripesWorkspaceContent>
      </Dialog>

      <Dialog open={newTopicOpen} onOpenChange={(open) => { setNewTopicOpen(open); if (!open) resetNewTopic(); }}>
        <StripesEditorContent onOpenAutoFocus={(event) => event.preventDefault()}>
          <DialogHeader><DialogTitle className="text-left text-base font-black text-[#102A43] lg:text-xl">{newTopicKind ? (newTopicKind === "idea" ? t("actionBoard.topic.newIdea") : newTopicKind === "decide" ? t("actionBoard.topic.newDecision") : t("actionBoard.topic.newTodo")) : t("actionBoard.topic.create")}</DialogTitle></DialogHeader>
          {!newTopicKind ? (
            <div className="grid gap-2.5 lg:gap-3.5">
              <button type="button" className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 text-left transition hover:bg-slate-50 lg:p-5" onClick={() => setNewTopicKind("idea")}>
                <div className="flex items-start gap-3 lg:gap-4"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-amber-50 text-amber-700 lg:h-12 lg:w-12 lg:rounded-2xl"><Lightbulb className="h-5 w-5 lg:h-6 lg:w-6" /></div><div><div className="text-sm font-black text-[#102A43] lg:text-base">{t("actionBoard.types.idea")}</div><div className="mt-0.5 text-[11px] font-semibold leading-relaxed text-slate-500 lg:text-sm">{t("actionBoard.topic.ideaHelp")}</div><div className="mt-1 text-[10px] font-bold text-slate-400 lg:text-xs">{t("actionBoard.topic.ideaExample")}</div></div></div>
              </button>
              <button type="button" className="rounded-2xl border border-amber-100 bg-amber-50/60 p-4 text-left transition hover:bg-amber-50 lg:p-5" onClick={() => setNewTopicKind("decide")}>
                <div className="flex items-start gap-3 lg:gap-4"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white text-amber-700 ring-1 ring-amber-100 lg:h-12 lg:w-12 lg:rounded-2xl"><Gavel className="h-5 w-5 lg:h-6 lg:w-6" /></div><div><div className="text-sm font-black text-[#102A43] lg:text-base">{t("actionBoard.workflow.decide")}</div><div className="mt-0.5 text-[11px] font-semibold leading-relaxed text-slate-500 lg:text-sm">{t("actionBoard.topic.decisionHelp")}</div><div className="mt-1 text-[10px] font-bold text-slate-400 lg:text-xs">{t("actionBoard.topic.decisionExample")}</div></div></div>
              </button>
              <button type="button" className="min-w-0 rounded-2xl border border-sky-100 bg-sky-50/60 p-4 text-left transition hover:bg-sky-50 lg:p-5" onClick={() => setNewTopicKind("action")}>
                <div className="flex items-start gap-3 lg:gap-4"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white text-sky-800 ring-1 ring-sky-100 lg:h-12 lg:w-12 lg:rounded-2xl"><Hand className="h-5 w-5 lg:h-6 lg:w-6" /></div><div><div className="text-sm font-black text-[#102A43] lg:text-base">{t("actionBoard.workflow.todo")}</div><div className="mt-0.5 text-[11px] font-semibold leading-relaxed text-slate-500 lg:text-sm">{t("actionBoard.topic.todoHelp")}</div><div className="mt-1 text-[10px] font-bold text-slate-400 lg:text-xs">{t("actionBoard.topic.todoExample")}</div></div></div>
              </button>
            </div>
          ) : (
            <div className="grid gap-3">
              <button type="button" className="w-fit text-[11px] font-black text-slate-500 lg:text-sm" onClick={() => setNewTopicKind(null)}>{t("actionBoard.backArrow")}</button>
              <div>
                <Label htmlFor="topic-title">{newTopicKind === "idea" ? t("actionBoard.topic.ideaQuestion") : newTopicKind === "decide" ? t("actionBoard.topic.decisionQuestion") : t("actionBoard.topic.todoQuestion")}</Label>
                <Textarea id="topic-title" value={newTitle} onChange={(event) => setNewTitle(event.target.value)} rows={2} maxLength={220} placeholder={newTopicKind === "idea" ? t("actionBoard.topic.ideaPlaceholder") : newTopicKind === "decide" ? t("actionBoard.topic.decisionPlaceholder") : t("actionBoard.topic.todoPlaceholder")} />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div><Label htmlFor="topic-tag"><Tag className="mr-1 inline h-3.5 w-3.5" />{t("actionBoard.card.tag")}</Label><select id="topic-tag" className="mt-1 h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={newCategory} onChange={(event) => setNewCategory(event.target.value)}><option value="">{t("actionBoard.common.none")}</option>{TAGS.map((item) => <option key={item} value={item}>{tagLabel(item)}</option>)}</select></div>
                <div><Label htmlFor="topic-due"><CalendarDays className="mr-1 inline h-3.5 w-3.5" />{t("actionBoard.card.due")}</Label><Input id="topic-due" type="date" value={newDueDate} onChange={(event) => setNewDueDate(event.target.value)} /></div>
              </div>
              {renderPeoplePicker(newPeopleKeys, setNewPeopleKeys)}
              <Button type="button" className="h-11 rounded-2xl font-black text-white lg:h-12 lg:text-base" style={{ backgroundColor: accent }} disabled={!newTitle.trim() || saving} onClick={() => void createTopic()}>{saving ? t("actionBoard.saving") : newTopicKind === "idea" ? t("actionBoard.topic.saveIdea") : newTopicKind === "decide" ? t("actionBoard.topic.continueToDecision") : t("actionBoard.topic.createTodo")}</Button>
            </div>
          )}
      ...
      <Dialog open={Boolean(decisionCardId)} onOpenChange={(open) => { if (!open) { setDecisionCardId(null); setDecisionStep(null); setDecisionEditingDecisionId(null); } }}>
        <StripesEditorContent className="box-border min-w-0 max-h-[90dvh] max-w-[calc(100vw-1rem)] overflow-x-hidden sm:w-[calc(100vw-2rem)] sm:max-w-lg lg:max-w-2xl" onOpenAutoFocus={(event) => event.preventDefault()}>
          <DialogHeader><DialogTitle className="text-left text-base font-black text-[#102A43] lg:text-xl">{decisionStep === "schedule" ? (decisionEditingDecisionId ? t("actionBoard.schedule.setup") : t("actionBoard.types.schedule")) : decisionStep ? t("actionBoard.decision.setup") : t("actionBoard.decision.kindQuestion")}</DialogTitle></DialogHeader>
          {!decisionStep ? (
            <div className="grid min-w-0 grid-cols-2 gap-2">
              <button type="button" className="min-w-0 rounded-2xl border border-amber-100 bg-amber-50/60 p-4 text-left lg:p-5" onClick={() => chooseDecisionType("vote")}><Vote className="h-5 w-5 text-amber-700" /><div className="mt-2 text-sm font-black text-[#102A43] lg:text-base">{t("actionBoard.types.vote")}</div><div className="mt-1 text-[11px] font-semibold text-slate-500 lg:text-sm">{t("actionBoard.decision.voteHelp")}</div></button>
              <button type="button" className="rounded-2xl border border-sky-100 bg-sky-50/60 p-4 text-left lg:p-5" onClick={() => chooseDecisionType("schedule")}><CalendarDays className="h-5 w-5 text-sky-700" /><div className="mt-2 text-sm font-black text-[#102A43] lg:text-base">{t("actionBoard.types.schedule")}</div><div className="mt-1 text-[11px] font-semibold text-slate-500 lg:text-sm">{t("actionBoard.schedule.findTogether")}</div></button>
              <button type="button" className="min-w-0 rounded-2xl border border-emerald-100 bg-emerald-50/60 p-4 text-left lg:p-5" onClick={() => chooseDecisionType("players")}><Users className="h-5 w-5 text-emerald-700" /><div className="mt-2 text-sm font-black text-[#102A43] lg:text-base">{t("actionBoard.types.players")}</div><div className="mt-1 text-[11px] font-semibold text-slate-500 lg:text-sm">{t("actionBoard.decision.playersHelp")}</div></button>
              <button type="button" className="min-w-0 rounded-2xl border border-amber-100 bg-amber-50/60 p-4 text-left lg:p-5" onClick={() => chooseDecisionType("equipment")}><ClipboardList className="h-5 w-5 text-amber-700" /><div className="mt-2 text-sm font-black text-[#102A43] lg:text-base">{t("actionBoard.types.equipment")}</div><div className="mt-1 text-[11px] font-semibold text-slate-500 lg:text-sm">{t("actionBoard.decision.equipmentHelp")}</div></button>
            </div>
          ) : (
            <div className="grid w-full max-w-full min-w-0 gap-3 overflow-x-hidden [&>*]:min-w-0">
              <button type="button" className="w-fit max-w-full text-[11px] font-black text-slate-500" onClick={() => setDecisionStep(null)}>{t("actionBoard.decision.changeType")}</button>

              {decisionStep === "vote" && <div className="grid w-full min-w-0 grid-cols-2 rounded-2xl bg-slate-100 p-1">
                <button type="button" className={`min-w-0 whitespace-normal rounded-xl px-2 py-2 text-xs font-semibold ${decisionMode === "vote" ? "bg-white text-amber-700 shadow-sm" : "text-slate-500"}`} onClick={() => setDecisionMode("vote")}><Vote className="mr-1 inline h-3.5 w-3.5" />{t("actionBoard.decision.voteTogether")}</button>
                <button type="button" className={`min-w-0 whitespace-normal rounded-xl px-2 py-2 text-xs font-semibold ${decisionMode === "recorded" ? "bg-white text-emerald-700 shadow-sm" : "text-slate-500"}`} onClick={() => setDecisionMode("recorded")}><Check className="mr-1 inline h-3.5 w-3.5" />{t("actionBoard.decision.recordDecision")}</button>
              </div>}

              {decisionMode === "recorded" ? (
                <>
                  {decisionPhaseNameRequired && <div><Label htmlFor="decision-recorded-name">{t("actionBoard.decision.name")}</Label><Input id="decision-recorded-name" value={decisionTitle} onChange={(event) => setDecisionTitle(event.target.value)} maxLength={120} placeholder={t("actionBoard.decision.namePlaceholder")} /></div>}
                  <div><Label htmlFor="decision-outcome">{t("actionBoard.decision.whatDecided")}</Label><Textarea id="decision-outcome" value={decisionOutcome} onChange={(event) => setDecisionOutcome(event.target.value)} rows={3} maxLength={300} placeholder={t("actionBoard.decision.outcomePlaceholder")} /></div>
                </>
              ) : (
                <>
                  {decisionStep === "schedule" ? <>
                    {!decisionEditingDecisionId ? (
                      <div className="grid gap-3">
                        <div>
                          <div className="text-sm font-semibold text-[#102A43]">{t("actionBoard.schedule.whoWillHost")}</div>
                          <div className="mt-1 text-[11px] font-normal text-slate-500">{t("actionBoard.schedule.hostHelp")}</div>
                        </div>
                        <div className="grid min-w-0 grid-cols-1 gap-2 min-[390px]:grid-cols-3">
                          <button type="button" className="min-w-0 whitespace-normal rounded-2xl border border-sky-200 bg-sky-50 px-2 py-3 text-center text-xs font-semibold text-sky-900" disabled={saving || !decisionSetupCard} onClick={() => { if (decisionSetupCard) void createScheduleForHost(decisionSetupCard, currentActor); }}>{t("actionBoard.people.me")}</button>
                          <button type="button" className={`min-w-0 whitespace-normal rounded-2xl border px-2 py-3 text-center text-xs font-semibold ${scheduleHostChoice === "person" ? "border-amber-300 bg-amber-50 text-amber-900" : "border-slate-200 bg-white text-slate-700"}`} onClick={() => setScheduleHostChoice("person")}>{t("actionBoard.schedule.someoneElse")}</button>
                          <button type="button" className="min-w-0 whitespace-normal rounded-2xl border border-slate-200 bg-white px-2 py-3 text-center text-xs font-semibold text-slate-700" disabled={saving || otherOrganizerPeople.length === 0} onClick={() => void requestScheduleHost("group")}>{t("actionBoard.schedule.askGroup")}</button>
                        </div>
                        {scheduleHostChoice === "person" && <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3">
                          <Label>{t("actionBoard.schedule.chooseHost")}</Label>
                          ...
                          <Button type="button" className="mt-3 h-10 w-full rounded-xl bg-amber-600 text-xs font-semibold text-white hover:bg-amber-700" disabled={!scheduleRequestedHostKey || saving} onClick={() => void requestScheduleHost("person")}>{t("actionBoard.schedule.askToHost")}</Button>
                        </div>}
                        <div className="text-[10px] font-normal leading-relaxed text-slate-400">{t("actionBoard.schedule.hostRequestHelp")}</div>
                      </div>
                    ) : (
                      <>
                        <div>
                          <Label htmlFor="schedule-title">{t("actionBoard.schedule.whatScheduling")} <span className="font-normal text-slate-400">{t("actionBoard.common.optional")}</span></Label>
                          <Input id="schedule-title" value={decisionTitle} onChange={(event) => setDecisionTitle(event.target.value)} maxLength={120} placeholder={decisionSetupCard?.title || t("actionBoard.schedule.boardMeetingPlaceholder")} />
                        </div>
                        <div className="rounded-2xl bg-sky-50 px-3 py-2.5 text-xs text-sky-900"><span className="font-semibold">{t("actionBoard.schedule.hostColon")}</span> {scheduleHostName}</div>
                        <div>
                          <Label>{t("actionBoard.schedule.participants")}</Label>
                          <div className="mt-1.5 flex rounded-2xl bg-slate-100 p-1">
                            <button type="button" className={`flex-1 rounded-xl px-3 py-2 text-xs font-semibold ${scheduleParticipantMode === "all" ? "bg-white text-[#102A43] shadow-sm" : "text-slate-500"}`} onClick={() => setScheduleParticipantMode("all")}>{t("actionBoard.people.all")}</button>
                            <button type="button" className={`flex-1 rounded-xl px-3 py-2 text-xs font-semibold ${scheduleParticipantMode === "selected" ? "bg-white text-[#102A43] shadow-sm" : "text-slate-500"}`} onClick={() => setScheduleParticipantMode("selected")}>{t("actionBoard.people.choose")}</button>
                          </div>
                          {scheduleParticipantMode === "selected" && <div className="mt-2">{renderPeoplePicker(decisionPeopleKeys, setDecisionPeopleKeys, t("actionBoard.people.whoShouldRespond"), [scheduleHostName])}</div>}
                          {scheduleParticipantMode === "all" && <div className="mt-1 text-[10px] font-normal text-slate-400">{t("actionBoard.schedule.allOtherOrganizers")}</div>}
                        </div>
                        <div>
                          <Label>{t("actionBoard.schedule.possibleDatesTimes")}</Label>
                          <div className="mt-1 text-[10px] font-normal text-slate-400">{t("actionBoard.schedule.timeOptionsHelp", { noneOption: t("actionBoard.schedule.noneAvailable") })}</div>
                          <div className="mt-1.5 grid gap-2">
                            {scheduleDates.map((group) => <div key={group.id} className="rounded-2xl border border-slate-200 bg-white p-2.5">
                              <div className="grid w-full min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-2">
                                ...
                                <button type="button" className="rounded-xl border border-slate-200 p-2 text-slate-400" onClick={() => setScheduleDates((current) => current.filter((item) => item.id !== group.id))} disabled={scheduleDates.length <= 1} aria-label={t("actionBoard.schedule.removeDate")}><Trash2 className="h-4 w-4" /></button>
                              </div>
                              <div className="mt-2 flex flex-wrap gap-2">
                                {group.times.map((time, timeIndex) => <div key={`${group.id}-${timeIndex}`} className="flex items-center gap-1">
                                  <Input
                                    ...
                                    maxLength={5}
                                    placeholder="19:30"
                                    aria-label={t("actionBoard.schedule.preferredTime")}
                                    className="h-10 w-[5.75rem] rounded-xl border-slate-200 bg-white px-2 text-center text-sm font-medium text-[#102A43]"
                                  />
                                  <button type="button" className="rounded-lg p-2 text-slate-400" onClick={() => updateScheduleDate(group.id, { times: group.times.filter((_, index) => index !== timeIndex) })} disabled={group.times.length <= 1} aria-label={t("actionBoard.schedule.removeTime")}><Trash2 className="h-3.5 w-3.5" /></button>
                                </div>)}
                                <button type="button" className="rounded-xl bg-slate-100 px-3 py-2 text-[11px] font-medium text-slate-600" onClick={() => updateScheduleDate(group.id, { times: [...group.times, ""] })}>{t("actionBoard.schedule.addTime")}</button>
                              </div>
                            </div>)}
                          </div>
                          <button type="button" className="mt-2 rounded-xl bg-slate-100 px-3 py-2 text-[11px] font-medium text-slate-600" onClick={() => setScheduleDates((current) => [...current, newScheduleDateGroup()])}>{t("actionBoard.schedule.addDate")}</button>
                        </div>
                        <div>
                          <Label htmlFor="schedule-locations">{t("actionBoard.schedule.locationOptions")}</Label>
                          <Textarea id="schedule-locations" value={scheduleLocations} onChange={(event) => setScheduleLocations(event.target.value)} rows={3} maxLength={500} placeholder={t("actionBoard.schedule.locationPlaceholder")} />
                          <div className="mt-1 text-[10px] font-normal text-slate-400">{t("actionBoard.schedule.locationOptionsHelp")}</div>
                        </div>
                      </>
                    )}
                  </> : <>
                    {decisionPhaseNameRequired && decisionStep !== "equipment" && <div><Label htmlFor="decision-name">{t("actionBoard.decision.name")}</Label><Input id="decision-name" value={decisionTitle} onChange={(event) => setDecisionTitle(event.target.value)} maxLength={120} placeholder={t("actionBoard.decision.namePlaceholder")} /></div>}

                    {decisionStep === "vote" && <>
                      <div className="rounded-2xl border border-amber-100 bg-amber-50/40 px-3 py-2.5">
                        <div className="text-[10px] font-medium uppercase tracking-wide text-amber-700">{t("actionBoard.decision.cardQuestion")}</div>
                      ...
                      <div className="grid gap-2">
                        {decisionQuestionsDraft.map((draft, questionIndex) => <div key={draft.id} className="w-full max-w-full min-w-0 rounded-2xl border border-slate-200 bg-white p-3">
                          <div className="flex items-center justify-between gap-2">
                            <Label htmlFor={`decision-question-${draft.id}`}>{t("actionBoard.decision.questionNumber", { number: questionIndex + 1 })}</Label>
                            {questionIndex > 0 && <button type="button" className="rounded-lg p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-600" onClick={() => setDecisionQuestionsDraft((current) => current.filter((item) => item.id !== draft.id))} aria-label={t("actionBoard.decision.removeQuestion", { number: questionIndex + 1 })}><Trash2 className="h-3.5 w-3.5" /></button>}
                          </div>
                          {questionIndex === 0 ? (
                            ...
                            <Textarea id={`decision-question-${draft.id}`} className="mt-1" value={draft.text} onChange={(event) => updateDraftQuestion(draft.id, { text: event.target.value })} rows={2} maxLength={220} placeholder={t("actionBoard.decision.anotherQuestionPlaceholder")} />
                          )}
                          <div className="mt-2 grid w-full min-w-0 grid-cols-1 gap-1 rounded-2xl bg-amber-50 p-1 min-[390px]:grid-cols-3">
                            <button type="button" className={`min-w-0 whitespace-normal rounded-xl px-2 py-2 text-[10px] font-semibold ${draft.kind === "yes-no-abstain" ? "bg-white text-amber-700 shadow-sm" : "text-amber-500"}`} onClick={() => updateDraftQuestion(draft.id, { kind: "yes-no-abstain" })}>{t("actionBoard.decision.yesNoAbstain")}</button>
                            <button type="button" className={`min-w-0 whitespace-normal rounded-xl px-2 py-2 text-[10px] font-semibold ${draft.kind === "choose-one" ? "bg-white text-amber-700 shadow-sm" : "text-amber-500"}`} onClick={() => updateDraftQuestion(draft.id, { kind: "choose-one" })}>{t("actionBoard.decision.chooseOne")}</button>
                            <button type="button" className={`min-w-0 whitespace-normal rounded-xl px-2 py-2 text-[10px] font-semibold ${draft.kind === "multi-select" ? "bg-white text-amber-700 shadow-sm" : "text-amber-500"}`} onClick={() => updateDraftQuestion(draft.id, { kind: "multi-select" })}>{t("actionBoard.decision.chooseSeveral")}</button>
                          </div>
                          {draft.kind !== "yes-no-abstain" && <div className="mt-2"><Label htmlFor={`decision-options-${draft.id}`}>{t("actionBoard.decision.choicesPerLine")}</Label><Textarea id={`decision-options-${draft.id}`} className="mt-1" value={draft.options} onChange={(event) => updateDraftQuestion(draft.id, { options: event.target.value })} rows={3} maxLength={700} placeholder={t("actionBoard.decision.optionsPlaceholder")} /></div>}
                          {draft.kind === "multi-select" && <div className="mt-2"><Label htmlFor={`decision-max-${draft.id}`}>{t("actionBoard.decision.maximumChoices")}</Label><Input id={`decision-max-${draft.id}`} type="number" min="1" max="16" value={draft.maxSelections} onChange={(event) => updateDraftQuestion(draft.id, { maxSelections: event.target.value })} inputMode="numeric" /></div>}
                        </div>)}
                      </div>
                      <button type="button" className="w-fit rounded-xl bg-amber-50 px-3 py-2 text-[11px] font-semibold text-amber-700 ring-1 ring-amber-100" onClick={() => setDecisionQuestionsDraft((current) => [...current, newDraftQuestion("")])}>{t("actionBoard.decision.addQuestion")}</button>
                      {renderVoterScope()}
                    </>}

                    {decisionStep === "players" && <>
                      <div className="rounded-2xl border border-amber-100 bg-amber-50/40 px-3 py-2.5">
                        <div className="text-[10px] font-medium uppercase tracking-wide text-amber-700">{t("actionBoard.decision.playerDecision")}</div>
                        <div className="mt-0.5 text-sm font-semibold leading-relaxed text-[#102A43]">{decisionSetupCard?.title}</div>
                      </div>
                      <div>
                        <div className="mb-1 flex items-center justify-between gap-2">
                          <Label htmlFor="player-search">{t("actionBoard.types.players")}</Label>
                          <div className="flex items-center gap-2 text-[10px] font-medium text-slate-500">
                            <span>{t("actionBoard.people.selectedCount", { count: selectedPlayerIds.length })}</span>
                            {selectedPlayerIds.length > 0 && <button type="button" className="text-slate-400 hover:text-slate-600" onClick={() => setSelectedPlayerIds([])}>{t("actionBoard.common.clear")}</button>}
                          </div>
                        </div>
                        <Input id="player-search" value={playerSearch} onChange={(event) => setPlayerSearch(event.target.value)} placeholder={t("actionBoard.people.searchRoster")} />
                        ...
                        })}{filteredPlayers.length === 0 && <div className="px-3 py-6 text-center text-xs font-medium text-slate-400">{t("actionBoard.people.noMatchingPlayers")}</div>}</div>
                      </div>
                      <div>
                        <div className="flex items-center justify-between gap-2"><Label htmlFor="player-answer-options">{t("actionBoard.decision.answerChoices")}</Label><button type="button" className="text-[10px] font-medium text-amber-700" onClick={() => setPlayerAnswerOptions(defaultAnswerOptions())}>{t("actionBoard.common.resetDefault")}</button></div>
                        <Textarea id="player-answer-options" value={playerAnswerOptions} onChange={(event) => setPlayerAnswerOptions(event.target.value)} rows={3} maxLength={180} placeholder={t("actionBoard.decision.defaultAnswerOptions")} />
                        <div className="mt-1 text-[10px] font-normal text-slate-400">{t("actionBoard.decision.playerAnswerHelp")}</div>
                      </div>
                      {renderVoterScope()}
                    </>}
                    ...
                    {decisionStep === "equipment" && <>
                      <div>
                        <Label htmlFor="equipment-card-title">{t("actionBoard.card.title")}</Label>
                        <Input
                          id="equipment-card-title"
                          value={decisionTitle}
                          onChange={(event) => setDecisionTitle(event.target.value)}
                          maxLength={220}
                          placeholder={decisionSetupCard?.title || t("actionBoard.decision.equipmentPlaceholder")}
                        />
                        <div className="mt-1 text-[10px] font-normal text-slate-400">{t("actionBoard.decision.equipmentTitleHelp")}</div>
                      </div>
                      ...
                      >
                        <div className="min-w-0">
                          <div className="text-xs font-semibold text-[#102A43]">{t("actionBoard.equipment.clubInventory")}</div>
                          <div className="mt-0.5 truncate text-[10px] font-medium text-slate-400">
                            {t("actionBoard.equipment.bagCount", { count: equipmentSnapshot?.bags.length || 0 })}
                            {equipmentSnapshot?.totals.length ? ` · ${equipmentSnapshot.totals.slice(0, 3).map((item) => t("actionBoard.equipment.inventoryItemQuantity", {
                              label: item.label,
                              quantity: formatNumber(getResolvedUiLocale(), item.quantity),
                            })).join(" · ")}` : ""}
                          </div>
                        </div>
                        ...
                        <div className="grid gap-2">{equipmentDraftItems.map((item, index) => {
                          ...
                          return <div key={item.id} className="min-w-0 rounded-2xl border border-slate-200 bg-white p-2.5">
                            <Input
                              ...
                              maxLength={120}
                              placeholder={t("actionBoard.equipment.itemNumber", { number: index + 1 })}
                            ...
                            <div className="mt-2 grid min-w-0 grid-cols-[5.25rem_minmax(0,1fr)_2.25rem_2.25rem] items-center gap-1.5">
                              <div className="grid h-9 grid-cols-[1.7rem_minmax(0,1fr)_1.7rem] overflow-hidden rounded-xl border border-slate-200 bg-white" aria-label={t("actionBoard.equipment.quantityForItem", { number: index + 1 })}>
                                <button type="button" className="flex items-center justify-center text-base font-medium text-slate-500 hover:bg-slate-50" onClick={() => updateEquipmentDraftItem(item.id, { quantity: String(Math.max(0, quantity - 1)) })} aria-label={t("actionBoard.equipment.decreaseQuantity", { number: index + 1 })}>−</button>
                                <input type="number" min="0" inputMode="numeric" value={item.quantity} onChange={(event) => updateEquipmentDraftItem(item.id, { quantity: event.target.value })} onBlur={() => { if (!item.quantity.trim()) updateEquipmentDraftItem(item.id, { quantity: "1" }); }} className="min-w-0 border-x border-slate-200 bg-transparent px-0 text-center text-sm font-semibold text-[#102A43] outline-none" aria-label={t("actionBoard.equipment.quantityValue", { number: index + 1 })} />
                                <button type="button" className="flex items-center justify-center text-base font-medium text-slate-500 hover:bg-slate-50" onClick={() => updateEquipmentDraftItem(item.id, { quantity: String(quantity + 1) })} aria-label={t("actionBoard.equipment.increaseQuantity", { number: index + 1 })}>+</button>
                              </div>
                              <Input value={item.price} onChange={(event) => updateEquipmentDraftItem(item.id, { price: event.target.value })} maxLength={32} inputMode="decimal" placeholder={t("actionBoard.equipment.price")} aria-label={t("actionBoard.equipment.priceForItem", { number: index + 1 })} className="h-9 min-w-0 px-2.5" />
                              <button type="button" className={`relative flex h-9 w-9 items-center justify-center rounded-xl border ${item.url.trim() && validHttpUrl(item.url) ? "border-sky-200 bg-sky-50 text-sky-700" : "border-slate-200 bg-white text-slate-500"}`} onClick={() => { setEquipmentLinkItemId(item.id); setEquipmentLinkDraft(item.url); }} aria-label={t("actionBoard.equipment.productLinkForItem", { number: index + 1 })} title={item.url ? t("actionBoard.equipment.editProductLink") : t("actionBoard.equipment.addProductLink")}><Link2 className="h-4 w-4" />{item.url.trim() && validHttpUrl(item.url) && <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-sky-700 text-white"><Check className="h-2.5 w-2.5" /></span>}</button>
                              <button type="button" className="flex h-9 w-8 items-center justify-center rounded-xl text-slate-400 hover:bg-red-50 hover:text-red-600" onClick={() => removeEquipmentDraftItem(item.id)} aria-label={t("actionBoard.equipment.removeItem", { number: index + 1 })}><Trash2 className="h-3.5 w-3.5" /></button>
                            </div>
                            {subtotal && <div className="mt-1.5 text-[9px] font-medium text-slate-400">{t("actionBoard.equipment.subtotalDot", { subtotal })}</div>}
                          </div>;
                        })}</div>
                        <button type="button" className="mt-2 w-fit rounded-xl bg-slate-100 px-3 py-2 text-[11px] font-semibold text-slate-600" onClick={() => setEquipmentPendingSubject("__blank__")}><Plus className="mr-1 inline h-3.5 w-3.5" />{t("actionBoard.equipment.addItem")}</button>
                      </div>
                      ...
                      >
                        <div className="min-w-0">
                          <div className="text-xs font-semibold text-[#102A43]">{t("actionBoard.decision.answerChoices")}</div>
                          <div className="mt-0.5 truncate text-[10px] font-medium text-slate-400">{answerLabels(equipmentAnswerOptions).join(" · ")}</div>
                        </div>
                        <ChevronRight className="h-4 w-4 shrink-0 text-slate-400" />
                      </button> : <div>
                        <Label htmlFor="equipment-max-selections">{t("actionBoard.equipment.maximumItems")}</Label>
              ...
              >
                {saving ? t("actionBoard.saving") : decisionMode === "recorded" ? t("actionBoard.decision.recordDecision") : decisionStep === "schedule" ? (scheduleNeedsPoll ? t("actionBoard.schedule.openAvailability") : t("actionBoard.schedule.save")) : t("actionBoard.vote.open")}
              </Button>}
            </div>
          )}
        </StripesEditorContent>
      ...
      <Dialog open={Boolean(equipmentPendingSubject)} onOpenChange={(open) => { if (!open) setEquipmentPendingSubject(""); }}>
        <DialogContent className="w-[calc(100%-2rem)] max-w-sm rounded-[1.75rem] p-4" onOpenAutoFocus={(event) => event.preventDefault()}>
          <DialogHeader><DialogTitle className="text-left text-base font-semibold text-[#102A43]">{equipmentPendingSubject === "__blank__" ? t("actionBoard.equipment.addAnotherQuestion") : t("actionBoard.equipment.addEquipmentQuestion")}</DialogTitle></DialogHeader>
          <div className="text-sm font-normal leading-relaxed text-slate-600">{equipmentPendingSubject === "__blank__" ? t("actionBoard.equipment.addNewHelp") : t("actionBoard.equipment.addNamedHelp", { name: equipmentPendingSubject })}</div>
          <div className="mt-3 flex gap-2">
            <Button type="button" variant="outline" className="h-10 flex-1 rounded-xl" onClick={() => setEquipmentPendingSubject("")}>{t("common.cancel")}</Button>
            <Button type="button" className="h-10 flex-1 rounded-xl bg-amber-600 font-semibold text-white hover:bg-amber-700" onClick={() => { if (equipmentPendingSubject === "__blank__") setEquipmentDraftItems((current) => [...current, newEquipmentDraftItem()]); else chooseEquipmentSubject(equipmentPendingSubject); setEquipmentPendingSubject(""); }}>{t("actionBoard.equipment.addItem")}</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={Boolean(equipmentLinkItemId)} onOpenChange={(open) => { if (!open) { setEquipmentLinkItemId(null); setEquipmentLinkDraft(""); } }}>
        <StripesSheetContent className="rounded-[1.75rem] sm:max-w-sm" onOpenAutoFocus={(event) => event.preventDefault()}>
          <DialogHeader><DialogTitle className="text-left text-base font-semibold text-[#102A43]">{t("actionBoard.equipment.productLink")}</DialogTitle></DialogHeader>
          <div className="grid gap-3">
            <Input type="url" inputMode="url" value={equipmentLinkDraft} onChange={(event) => setEquipmentLinkDraft(event.target.value)} placeholder={t("actionBoard.link.urlPlaceholder")} />
            {equipmentLinkDraft.trim() && !validHttpUrl(equipmentLinkDraft) && <div className="text-[10px] font-medium text-red-500">{t("actionBoard.link.validHttpHelp")}</div>}
            <div className="flex gap-2">
              {equipmentLinkItemId && equipmentDraftItems.find((item) => item.id === equipmentLinkItemId)?.url && <Button type="button" variant="outline" className="h-10 rounded-xl text-red-600" onClick={() => { updateEquipmentDraftItem(equipmentLinkItemId, { url: "" }); setEquipmentLinkItemId(null); setEquipmentLinkDraft(""); }}>{t("common.remove")}</Button>}
              <Button type="button" className="h-10 flex-1 rounded-xl bg-[#102A43] font-semibold text-white" disabled={Boolean(equipmentLinkDraft.trim()) && !validHttpUrl(equipmentLinkDraft)} onClick={() => { if (equipmentLinkItemId) updateEquipmentDraftItem(equipmentLinkItemId, { url: equipmentLinkDraft.trim() }); setEquipmentLinkItemId(null); setEquipmentLinkDraft(""); }}>{t("actionBoard.link.save")}</Button>
            </div>
          </div>
        </StripesSheetContent>
      </Dialog>
          ...
          {addToCardCard && (
            <div className="grid gap-1.5">
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block text-sm font-semibold text-[#102A43]">{t("actionBoard.note.label")}</span>
                  <span className="block truncate text-[11px] font-normal text-slate-400">
                    {addToCardCard.note?.trim() ? t("actionBoard.note.editExisting") : t("actionBoard.note.addContext")}
                  </span>
                </span>
              </button>
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block text-sm font-semibold text-[#102A43]">{t("actionBoard.people.assignees")}</span>
                  <span className="block truncate text-[11px] font-normal text-slate-400">
                    {(addToCardCard.people?.length || 0) > 0
                      ? t("actionBoard.people.assignedCount", { count: addToCardCard.people?.length })
                      : t("actionBoard.people.chooseHandler")}
                  </span>
                </span>
              </button>
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block text-sm font-semibold text-[#102A43]">{t("actionBoard.card.dueDate")}</span>
                  <span className="block truncate text-[11px] font-normal text-slate-400">
                    {addToCardCard.dueDate ? dueText(addToCardCard.dueDate) : t("actionBoard.card.setDeadline")}
                  </span>
                </span>
              </button>
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block text-sm font-semibold text-[#102A43]">{t("actionBoard.link.label")}</span>
                  <span className="block truncate text-[11px] font-normal text-slate-400">
                    {(addToCardCard.links?.length || 0) > 0
                      ? t("actionBoard.link.addedCount", { count: addToCardCard.links?.length, total: 5 })
                      : t("actionBoard.link.addWebDocument")}
                  </span>
                </span>
              </button>
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block text-sm font-semibold text-[#102A43]">{t("actionBoard.files.file")}</span>
                  <span className="block truncate text-[11px] font-normal text-slate-400">
                    {!isSharedRoster
                      ? t("actionBoard.files.sharedRosterRequired")
                      : !online
                        ? t("actionBoard.files.signIn")
                        : fileUploadingCardId === addToCardCard.id
                          ? t("actionBoard.files.uploadingShort")
                          : cardFileResources(addToCardCard.id).length >= STRIPES_FILE_MAX_CARD_ATTACHMENTS
                            ? t("actionBoard.files.addedCount", { count: STRIPES_FILE_MAX_CARD_ATTACHMENTS, total: STRIPES_FILE_MAX_CARD_ATTACHMENTS })
                            : cardFileResources(addToCardCard.id).length > 0
                              ? t("actionBoard.files.addedCount", { count: cardFileResources(addToCardCard.id).length, total: STRIPES_FILE_MAX_CARD_ATTACHMENTS })
                              : t("actionBoard.files.uploadDevice")}
                  </span>
                </span>
              </button>
            </div>
      ...
      <Dialog open={Boolean(linkCardId)} onOpenChange={(open) => { if (!open) setLinkCardId(null); }}>
        <StripesSheetContent onOpenAutoFocus={(event) => event.preventDefault()}>
          <DialogHeader><DialogTitle className="text-left text-base font-black text-[#102A43]">{t("actionBoard.link.add")}</DialogTitle></DialogHeader>
          <div className="grid gap-3">
            <div><Label htmlFor="link-url">{t("actionBoard.link.paste")}</Label><Input id="link-url" type="url" inputMode="url" value={linkUrl} onChange={(event) => setLinkUrl(event.target.value)} placeholder={t("actionBoard.link.urlPlaceholder")} /></div>
            <div><Label htmlFor="link-label">{t("actionBoard.link.labelText")} <span className="font-semibold text-slate-400">{t("actionBoard.common.optional")}</span></Label><Input id="link-label" value={linkLabel} onChange={(event) => setLinkLabel(event.target.value)} maxLength={80} placeholder={validHttpUrl(linkUrl) ? providerLabel(linkUrl) : t("actionBoard.link.labelPlaceholder")} /></div>
            <Button type="button" className="h-11 rounded-2xl font-black text-white" style={{ backgroundColor: accent }} disabled={!validHttpUrl(linkUrl) || saving} onClick={() => void addLink()}>{saving ? t("actionBoard.saving") : t("actionBoard.link.add")}</Button>
          </div>
        </StripesSheetContent>
      </Dialog>
      ...
      <Dialog open={Boolean(moveCardId)} onOpenChange={(open) => { if (!open) setMoveCardId(null); }}>
        <DialogContent className="w-[calc(100%-2rem)] max-w-sm rounded-[1.75rem] p-4">
          <DialogHeader><DialogTitle className="text-left text-base font-semibold text-[#102A43]">{t("actionBoard.card.move")}</DialogTitle></DialogHeader>
          <div className="mt-1 text-xs font-medium text-slate-500">{t("actionBoard.card.moveHelp")}</div>
          <div className="mt-3 grid grid-cols-2 gap-2">
            {([
              ["ideas", t("actionBoard.workflow.ideas"), Lightbulb],
              ["deciding", t("actionBoard.workflow.decide"), Gavel],
              ["action", t("actionBoard.workflow.todo"), ClipboardList],
              ["done", t("actionBoard.workflow.done"), Check],
              ...
              return <button key={stage} type="button" disabled={current} className={`flex h-12 items-center justify-center gap-2 rounded-2xl border text-sm font-semibold ${current ? "border-slate-200 bg-slate-100 text-slate-400" : "border-slate-200 bg-white text-[#102A43] active:bg-slate-50"}`} onClick={() => void moveCardFromPicker(stage)}><Icon className="h-4 w-4" />{current ? t("actionBoard.card.currentColumn", { column: label }) : label}</button>;
            })}
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={Boolean(actionCardId)} onOpenChange={(open) => { if (!open) setActionCardId(null); }}>
        <StripesSheetContent onOpenAutoFocus={(event) => event.preventDefault()}>
          <DialogHeader><DialogTitle className="text-left text-base font-semibold text-[#102A43]">{t("actionBoard.action.todoDetails")}</DialogTitle></DialogHeader>
          <div className="grid gap-3">
            <div><Label htmlFor="action-text">{t("actionBoard.action.whatNext")}</Label><Textarea id="action-text" value={actionText} onChange={(event) => setActionText(event.target.value)} rows={2} maxLength={220} /></div>
            {renderPeoplePicker(actionPeopleKeys, setActionPeopleKeys, t("actionBoard.people.whoHandles"))}
            <Button type="button" className="h-11 rounded-2xl bg-sky-700 font-black text-white hover:bg-sky-800" disabled={!actionText.trim() || saving} onClick={() => void addAction()}>{saving ? t("actionBoard.saving") : t("actionBoard.action.saveDetails")}</Button>
          </div>
        </StripesSheetContent>
      </Dialog>
      ...
      <Dialog open={equipmentAnswersOpen} onOpenChange={(open) => setEquipmentAnswersOpen(open)}>
        <DialogContent className="w-[calc(100%-2rem)] max-w-sm rounded-[1.75rem] p-4" onOpenAutoFocus={(event) => event.preventDefault()}>
          <DialogHeader><DialogTitle className="text-left text-base font-semibold text-[#102A43]">{t("actionBoard.decision.answerChoices")}</DialogTitle></DialogHeader>
          <div className="grid gap-3">
            <Textarea value={equipmentAnswerOptionsDraft} onChange={(event) => setEquipmentAnswerOptionsDraft(event.target.value)} rows={4} maxLength={180} placeholder={t("actionBoard.decision.defaultAnswerOptions")} />
            <div className="text-[10px] font-normal text-slate-400">{t("actionBoard.equipment.answerHelp")}</div>
            <div className="flex gap-2">
              <Button type="button" variant="outline" className="h-10 rounded-xl" onClick={() => setEquipmentAnswerOptionsDraft(defaultAnswerOptions())}>{t("actionBoard.common.reset")}</Button>
              <Button type="button" className="h-10 flex-1 rounded-xl bg-amber-600 font-semibold text-white hover:bg-amber-700" disabled={answerLabels(equipmentAnswerOptionsDraft).length < 2} onClick={() => { setEquipmentAnswerOptions(equipmentAnswerOptionsDraft); setEquipmentAnswersOpen(false); }}>{t("common.save")}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={Boolean(votingCard && votingDecision)} onOpenChange={(open) => { if (!open) { setVotingCardId(null); setVotingDecisionId(null); setSelectedVoteAnswers({}); } }}>
        <StripesSheetContent className="box-border min-w-0 max-h-[90dvh] max-w-[calc(100vw-1rem)] overflow-x-hidden overflow-y-auto sm:max-w-md">
          <DialogHeader><DialogTitle className="text-left text-base font-semibold text-[#102A43]">{votingDecision?.kind === "schedule" ? t("actionBoard.vote.scheduleResponse") : votingDecision?.decisionType === "players" ? t("actionBoard.decision.playerDecision") : t("actionBoard.vote.inStripes")}</DialogTitle></DialogHeader>
          {votingDecision && <div className="grid w-full max-w-full min-w-0 gap-3 [&>*]:min-w-0">
            ...
            {votingDecision.hostName && <div className="rounded-xl bg-sky-50 px-3 py-2 text-[11px] font-bold text-sky-800">{t("actionBoard.schedule.hostLabel")} <span className="font-semibold">{votingDecision.hostName}</span></div>}
            {votingDecision.decisionType === "equipment" && <button
              ...
              disabled={!onOpenEquipmentInventory}
            >
              <div className="min-w-0"><div className="text-xs font-semibold text-[#102A43]">{t("actionBoard.equipment.clubInventory")}</div><div className="mt-0.5 truncate text-[10px] font-medium text-slate-400">{t("actionBoard.equipment.bagCount", { count: equipmentSnapshot?.bags.length || 0 })}{equipmentSnapshot?.totals.length ? ` · ${equipmentSnapshot.totals.slice(0, 3).map((item) => t("actionBoard.equipment.inventoryItemQuantity", {
                label: item.label,
                quantity: formatNumber(getResolvedUiLocale(), item.quantity),
              })).join(" · ")}` : ""}</div></div>
              <ChevronRight className="h-4 w-4 shrink-0 text-slate-400" />
            ...
            {(votingDecision.decisionType === "players" || (votingDecision.decisionType === "equipment" && votingDecision.equipmentVoteMode === "rate")) ? (
                  ...
                  return <div key={question.id} className={`px-3 py-2.5 ${questionIndex ? "border-t border-slate-100" : ""}`}>
                    <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-start">
                      <div className="min-w-0 sm:flex-1">
                        ...
                        {votingDecision.decisionType === "equipment" && (question.itemQuantity !== undefined || question.itemPrice || question.itemUrl) && <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-[9px] font-normal text-slate-400">{question.itemQuantity !== undefined && <span>{t("actionBoard.equipment.quantity", { count: question.itemQuantity })}</span>}{question.itemPrice && <span>{question.itemPrice}</span>}{question.itemPrice && question.itemQuantity !== undefined && <span className="font-medium text-slate-500">{t("actionBoard.equipment.subtotal", { subtotal: equipmentSubtotal(question.itemPrice, question.itemQuantity) })}</span>}{question.itemUrl && <a href={question.itemUrl} target="_blank" rel="noreferrer" className="text-sky-700 underline" onClick={(event) => event.stopPropagation()}>{t("actionBoard.link.open")}</a>}</div>}
                      </div>
              ...
              <div className="grid gap-3">{decisionQuestions(votingDecision).map((question, questionIndex) => {
                ...
                return <div key={question.id} className={decisionQuestions(votingDecision).length > 1 ? "rounded-2xl border border-slate-200 bg-white p-3" : ""}>
                  <div className="whitespace-normal break-words text-sm font-medium leading-relaxed text-[#102A43]">{decisionQuestions(votingDecision).length > 1 ? t("actionBoard.decision.numberedQuestion", { number: formatNumber(getResolvedUiLocale(), questionIndex + 1), question: question.text }) : question.text}</div>
                  {votingDecision.kind === "schedule" && question.scheduleRole === "time" && <div className="mt-1 text-[10px] font-normal text-slate-500">{t("actionBoard.vote.scheduleTimeHelp", { noneOption: t("actionBoard.schedule.noneAvailable") })}</div>}
                  {votingDecision.kind === "schedule" && question.scheduleRole === "location" && <div className="mt-1 text-[10px] font-normal text-slate-500">{t("actionBoard.vote.scheduleLocationHelp")}</div>}
                  {votingDecision.kind !== "schedule" && question.kind === "multi-select" && <div className="mt-1 text-[10px] font-medium text-slate-500">{t("actionBoard.vote.chooseUpTo", { count: question.maxSelections || question.options.length })}</div>}
                  <div className="mt-2 grid gap-2">{question.options.map((option) => {
                    ...
                    return <div key={option.id} className="rounded-2xl border border-slate-200 bg-white p-1">
                      <button type="button" className={`w-full rounded-xl px-2 py-2 text-left text-sm font-medium ${selected ? "bg-amber-50 text-amber-800" : "text-[#102A43]"}`} onClick={() => toggleVoteOption(question, option.id)}>
                        <div className="flex items-center gap-2"><span className="min-w-0 flex-1 break-words">{selected && <Check className="mr-1 inline h-4 w-4" />}{option.label}{option.quantity !== undefined && <span className="ml-1 text-[10px] font-normal text-slate-400">× {formatNumber(getResolvedUiLocale(), option.quantity)}</span>}</span><span className="shrink-0 text-right">{option.price && <span className="block text-xs font-medium text-slate-500">{option.price}</span>}{option.price && option.quantity !== undefined && <span className="block text-[9px] font-medium text-slate-400">{t("actionBoard.equipment.subtotal", { subtotal: equipmentSubtotal(option.price, option.quantity) })}</span>}</span></div>
                      </button>
                      {option.url && <a href={option.url} target="_blank" rel="noreferrer" className="block px-2 pb-2 text-[10px] font-medium text-sky-700 underline">{t("actionBoard.equipment.openProductLink")}</a>}
                    </div>;
                  })}</div>
                </div>;
              })}</div>
            )}
            <div className="text-[10px] font-normal leading-snug text-slate-500">{votingDecision.kind === "schedule" ? t("actionBoard.vote.schedulePrivacy") : votingDecision.decisionType === "players" ? t("actionBoard.vote.playerPrivacy") : t("actionBoard.vote.privacy")}</div>
            <Button type="button" className="h-11 rounded-2xl bg-amber-600 font-semibold text-white" disabled={decisionQuestions(votingDecision).some((question) => !(selectedVoteAnswers[question.id] || []).length) || voteSubmitting} onClick={() => void submitVote()}>{voteSubmitting ? t("actionBoard.vote.recording") : votingDecision.ballots?.some((ballot) => ballot.voterHash === currentVoterHash) ? t("actionBoard.vote.update") : t("actionBoard.vote.submit")}</Button>
          </div>}
        </StripesSheetContent>
      </Dialog>
      ...
      <Dialog open={Boolean(finalizeScheduleCard && finalizeScheduleDecision)} onOpenChange={(open) => { if (!open) { setFinalizeScheduleCardId(null); setFinalizeScheduleDecisionId(null); } }}>
        <StripesSheetContent className="max-h-[90dvh] overflow-y-auto" onOpenAutoFocus={(event) => event.preventDefault()}>
          <DialogHeader><DialogTitle className="text-left text-base font-semibold text-[#102A43]">{t("actionBoard.schedule.finalize")}</DialogTitle></DialogHeader>
          {finalizeScheduleDecision && <div className="grid gap-3">
            <div className="rounded-2xl bg-sky-50 px-3 py-2.5 text-[11px] font-normal text-sky-900">{t("actionBoard.schedule.finalizeHelp")}</div>
            <div>
              <Label htmlFor="final-schedule-time">{t("actionBoard.schedule.dateTime")}</Label>
              <select id="final-schedule-time" className="mt-1 h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-medium text-[#102A43]" value={finalScheduleTime} onChange={(event) => setFinalScheduleTime(event.target.value)}>
                <option value="">{t("actionBoard.schedule.chooseFinalTime")}</option>
                {(finalizeScheduleDecision.scheduleTimeValues || []).map((value) => <option key={value} value={value}>{scheduleLabel(value)}</option>)}
              </select>
            </div>
            <div>
              <Label htmlFor="final-schedule-location">{t("actionBoard.schedule.location")}</Label>
              <select id="final-schedule-location" className="mt-1 h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm font-medium text-[#102A43]" value={finalScheduleLocation} onChange={(event) => setFinalScheduleLocation(event.target.value)}>
                <option value="">{t("actionBoard.schedule.chooseFinalLocation")}</option>
                {(finalizeScheduleDecision.scheduleLocationOptions || []).map((value) => <option key={value} value={value}>{value}</option>)}
              </select>
            </div>
            <div>
              <Label htmlFor="final-schedule-link">{t("actionBoard.schedule.meetingLink")} <span className="font-normal text-slate-400">{t("actionBoard.common.optional")}</span></Label>
              <Input id="final-schedule-link" type="url" inputMode="url" value={finalScheduleMeetingUrl} onChange={(event) => setFinalScheduleMeetingUrl(event.target.value)} placeholder={t("actionBoard.schedule.meetingLinkPlaceholder")} />
            </div>
            <Button type="button" className="h-11 rounded-2xl bg-sky-700 font-semibold text-white hover:bg-sky-800" disabled={saving || !finalScheduleTime || !finalScheduleLocation || (Boolean(finalScheduleMeetingUrl.trim()) && !validHttpUrl(finalScheduleMeetingUrl))} onClick={() => void finalizeSchedule()}>{saving ? t("actionBoard.saving") : t("actionBoard.schedule.confirm")}</Button>
          </div>}
        </StripesSheetContent>
      </Dialog>
      ...
      <Dialog open={Boolean(outcomeCardId && outcomeDecisionId)} onOpenChange={(open) => { if (!open) { setOutcomeCardId(null); setOutcomeDecisionId(null); } }}>
        <StripesSheetContent onOpenAutoFocus={(event) => event.preventDefault()}>
          <DialogHeader><DialogTitle className="text-left text-base font-black text-[#102A43]">{t("actionBoard.decision.note")}</DialogTitle></DialogHeader>
          <div className="grid gap-3">
            <div><Label htmlFor="outcome-text">{t("actionBoard.decision.optionalNote")}</Label><Textarea id="outcome-text" value={outcomeText} onChange={(event) => setOutcomeText(event.target.value)} rows={3} maxLength={300} placeholder={t("actionBoard.decision.notePlaceholder")} /></div>
            <Button type="button" className="h-11 rounded-2xl bg-emerald-600 font-black text-white" disabled={saving} onClick={() => void saveOutcome()}>{saving ? t("actionBoard.saving") : outcomeText.trim() ? t("actionBoard.decision.saveNote") : t("actionBoard.decision.removeNote")}</Button>
          </div>
        </StripesSheetContent>
      </Dialog>
      ...
      <Dialog open={Boolean(notifyCardId && notifyTarget)} onOpenChange={(open) => { if (!open) closeNotify(); }}>
        <StripesEditorContent className="max-h-[90dvh] overflow-y-auto lg:max-w-lg" onOpenAutoFocus={(event) => event.preventDefault()}>
          <DialogHeader><DialogTitle className="text-left text-base font-black text-[#102A43] lg:text-xl">{t("actionBoard.notification.notifyOrganizers")}</DialogTitle></DialogHeader>
          {notifyTarget && <div className="grid gap-4">
            <div className="rounded-2xl bg-slate-50 px-3 py-3 ring-1 ring-slate-100">
              ...
              <div className="mt-1 text-[10px] font-semibold text-slate-500">{t("actionBoard.notification.oneEmailHelp")}</div>
            </div>

            <div>
              <Label>{t("actionBoard.notification.who")}</Label>
              <div className="mt-1.5 flex rounded-2xl bg-slate-100 p-1">
                <button type="button" className={`flex-1 rounded-xl px-3 py-2 text-xs font-black ${notifyRecipientMode === "all" ? "bg-white text-[#102A43] shadow-sm" : "text-slate-500"}`} onClick={() => setNotifyRecipientMode("all")}>{t("actionBoard.people.allOrganizers")}</button>
                <button type="button" className={`flex-1 rounded-xl px-3 py-2 text-xs font-black ${notifyRecipientMode === "selected" ? "bg-white text-[#102A43] shadow-sm" : "text-slate-500"}`} onClick={() => setNotifyRecipientMode("selected")}>{t("actionBoard.people.selected")}</button>
              </div>
              ...
              {!otherOrganizerPeople.length && <div className="mt-2 text-[11px] font-semibold text-amber-700">{t("actionBoard.people.noOtherSignedInOrganizers")}</div>}
            </div>

            <div>
              <Label>{t("actionBoard.notification.delivery")}</Label>
              <div className="mt-1.5 rounded-2xl border border-amber-200 bg-amber-50 px-3 py-3 text-amber-800">
                <Mail className="h-4 w-4" />
                <div className="mt-1 text-xs font-black">{t("actionBoard.notification.email")} <Check className="ml-1 inline h-3.5 w-3.5" /></div>
                <div className="mt-0.5 text-[9px] font-semibold opacity-75">{notifyTarget.topicAlreadyNotified ? t("actionBoard.notification.continuesThread") : t("actionBoard.notification.startsThread")}</div>
              </div>
            </div>

            <div>
              <Label htmlFor="notify-message">{t("actionBoard.notification.message")} <span className="font-semibold text-slate-400">{t("actionBoard.common.optional")}</span></Label>
              <Textarea id="notify-message" value={notifyMessage} onChange={(event) => setNotifyMessage(event.target.value)} rows={2} maxLength={500} placeholder={t("actionBoard.notification.messagePlaceholder")} />
            </div>
            ...
            <Button type="button" className="h-11 rounded-2xl bg-[#102A43] font-black text-white lg:h-12 lg:text-base" disabled={notifySending || !notifyEmailsToSend.length} onClick={() => void sendNotification()}>
              <Bell className="mr-1.5 h-4 w-4" />{notifySending ? t("actionBoard.notification.notifying") : t("actionBoard.notification.notifyCount", { count: notifyEmailsToSend.length })}
            </Button>
          </div>}
        </StripesEditorContent>
      </Dialog>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <StripesSheetContent onOpenAutoFocus={(event) => event.preventDefault()}>
          <DialogHeader><DialogTitle className="text-left text-base font-semibold text-[#102A43]">{editSection === "note" ? t("actionBoard.note.label") : editSection === "assignees" ? t("actionBoard.people.assignees") : editSection === "due" ? t("actionBoard.card.dueDate") : t("actionBoard.card.edit")}</DialogTitle></DialogHeader>
          <div className="grid gap-3">
            {editSection === "all" && <div><Label htmlFor="edit-title">{t("actionBoard.card.titleLabel")}</Label><Textarea id="edit-title" value={editTitle} onChange={(event) => setEditTitle(event.target.value)} rows={2} maxLength={220} /></div>}
            {(editSection === "all" || editSection === "note") && <div><Label htmlFor="edit-note">{t("actionBoard.note.label")} <span className="font-normal text-slate-400">{t("actionBoard.common.optional")}</span></Label><Textarea id="edit-note" value={editNote} onChange={(event) => setEditNote(event.target.value)} rows={4} maxLength={1200} placeholder={t("actionBoard.note.editPlaceholder")} /></div>}
            {editSection === "all" && <div className="grid grid-cols-2 gap-2">
              <div><Label htmlFor="edit-tag">{t("actionBoard.card.tag")}</Label><select id="edit-tag" className="mt-1 h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={editCategory} onChange={(event) => setEditCategory(event.target.value)}><option value="">{t("actionBoard.common.none")}</option>{TAGS.map((item) => <option key={item} value={item}>{tagLabel(item)}</option>)}</select></div>
              <div><Label htmlFor="edit-due">{t("actionBoard.card.due")}</Label><input id="edit-due" type="date" className="mt-1 h-10 w-full min-w-0 rounded-md border border-input bg-background px-3 text-sm" value={editDueDate} onChange={(event) => setEditDueDate(event.target.value)} /></div>
            </div>}
            {editSection === "due" && <div><Label htmlFor="edit-due-only">{t("actionBoard.card.dueDate")} <span className="font-normal text-slate-400">{t("actionBoard.common.optional")}</span></Label><Input id="edit-due-only" type="date" value={editDueDate} onChange={(event) => setEditDueDate(event.target.value)} /></div>}
            {(editSection === "all" || editSection === "assignees") && renderPeoplePicker(editPeopleKeys, setEditPeopleKeys, t("actionBoard.people.assignees"))}
            <div className="flex gap-2">
              {editSection === "all" && <Button type="button" variant="outline" className="h-11 rounded-2xl text-red-700" onClick={() => void removeCard()}><Trash2 className="mr-1 h-4 w-4" />{t("common.delete")}</Button>}
              {editSection !== "all" && <Button type="button" variant="outline" className="h-11 rounded-2xl" onClick={() => setEditOpen(false)}>{t("common.cancel")}</Button>}
              <Button type="button" className="h-11 flex-1 rounded-2xl font-semibold text-white" style={{ backgroundColor: accent }} disabled={!editTitle.trim() || saving} onClick={() => void saveEditedCard()}>{saving ? t("actionBoard.saving") : t("common.save")}</Button>
            </div>
          </div>
        </StripesSheetContent>
      </Dialog>
        ...
        <DialogContent className="stripes-type-ui max-w-sm rounded-3xl" onOpenAutoFocus={(event) => event.preventDefault()}>
          <DialogHeader><DialogTitle className="text-left text-base font-black text-[#102A43]">{t("actionBoard.settings.title")}</DialogTitle></DialogHeader>
          <div className="grid gap-4">
            <div><Label htmlFor="board-name">{t("actionBoard.settings.customName")} <span className="font-semibold text-slate-400">{t("actionBoard.settings.customNameHelp")}</span></Label><Input id="board-name" value={boardNameDraft} onChange={(event) => changeBoardName(event.target.value)} maxLength={80} placeholder={t("actionBoard.settings.namePlaceholder")} /></div>
            <div className="rounded-2xl bg-slate-50 p-3 text-[10px] font-semibold leading-relaxed text-slate-500 ring-1 ring-slate-100">
              {t("actionBoard.settings.notificationHelp")}
            </div>
            {notifyError && <div className="rounded-xl bg-red-50 px-3 py-2 text-[11px] font-bold text-red-700">{notifyError}</div>}
          </div>
        </DialogContent>
