  ...
  return (
    <div className="flex flex-col min-h-[100dvh] bg-background w-full max-w-md mx-auto relative shadow-2xl overflow-hidden">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
        <header className="sticky top-0 z-30 bg-white/95 backdrop-blur border-b border-border px-4 pt-3 pb-2 shadow-sm">
          <div className="flex items-center justify-between gap-3 px-1 pb-2">
            <div className="min-w-0 flex-1">
              {isEditingGroupName ? (
                ...
                <button type="button" onClick={startGroupNameEdit} className="group flex items-center gap-2 text-left min-w-0 max-w-full active:scale-[0.99] transition-transform">
                  <span
                    className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-2xl border-2 bg-white shadow-sm"
                    style={{ borderColor: isDefaultHeaderColor ? "#E2E8F0" : headerColor }}
                  >
                    <img src={fairTeamsLogo} alt="" className="h-full w-full object-contain p-0.5" />
                  </span>
                  <h1 className="min-w-0 flex-1 truncate text-[15px] font-black leading-tight tracking-tight text-[#102A43]">{groupName}</h1>
            ...
            {!isEditingGroupName && (
              <div className="flex items-center gap-1.5 shrink-0">
                {activeTab === "players" && (
                  <>
                    <label className="relative h-7 w-7 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center cursor-pointer active:scale-95 transition-transform" title="Pick team color">
                    ...
                    <Button
                      ...
                      variant="secondary"
                      size="sm"
                      className="h-7 rounded-lg bg-slate-100 px-2 text-[10px] font-black text-[#102A43]/75 border border-slate-200"
                      onClick={() => setHeaderColor(DEFAULT_HEADER_COLOR)}
                      title="No color / white"
                      disabled={isDefaultHeaderColor}
                    >
                      White
                    </Button>
          ...
          <div
            ...
            style={{ backgroundColor: isDefaultHeaderColor ? "#E2E8F0" : headerColor }}
          />
