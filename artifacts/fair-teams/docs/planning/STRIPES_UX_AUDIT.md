...
Status: **experimental evidence system**

Branch target: `experiment/joongpt-prototype`
Canonical production roadmap: unchanged until explicit reconciliation
...
Everything judged worth keeping must eventually be carried into the real Stripes
project deliberately:

1. Capture the experimental app.
2. Audit screenshots, accessibility snapshots, browser traces and metrics.
3. Classify each experimental idea as **Keep**, **Change** or **Remove**.
4. Approve the target interaction and visual hierarchy.
5. Reconcile approved decisions into the canonical `STRIPES_ROADMAP.md`.
6. Implement the production-quality version on the real project/main path with
   its normal engineering gates.

The experimental branch is not merged wholesale merely because it looks useful.
Proven product decisions may be adopted while prototype-only code is rewritten
or discarded.

## What the audit captures

The first audit matrix covers:
...
- current Player Model manager;
- preset editor;
- existing Player Setup;
- Player Setup after selecting a preset;
- experimental batch rating;
- Teams setup;
- generated teams;
- empty custom roster;
