...
import { createCustomRosterPlayerModel } from "../../src/lib/newRosterSetup.ts";
import {
...
} from "../../src/lib/rosterPlayerModel.ts";
...
function profileValue(index: number, offset: number) {
  return Math.max(1, Math.min(10, Math.round((4.5 + (index % 6) * 0.6 + offset) * 2) / 2));
}

function auditPlayer(name: string, index: number, presetIds: string[] = []) {
  ...
  return {
    ...
    attack: profileValue(index, index % 3 === 0 ? 1.5 : 0),
    passing: profileValue(index, index % 4 === 1 ? 1 : 0),
    stamina: profileValue(index, index % 5 === 2 ? 1.5 : 0),
    defense: profileValue(index, index % 3 === 2 ? 1.5 : -0.5),
    physical: profileValue(index, index % 4 === 3 ? 1 : 0),
    speed: profileValue(index, index % 5 === 0 ? 1.5 : 0),
    teamPlay: 2,
    profilePresetIds: presetIds,
    profileFineTuned: index === 3,
    todayStatus: "here",
    attending: true,
    isNew: index >= 12,
...
function preset(
  ...
  name: string,
  description: string,
  iconKey: RosterPlayerPreset["iconKey"],
  values: Partial<Record<PlayerProfileSlot, number>>,
): RosterPlayerPreset {
  const slots: PlayerProfileSlot[] = ["attack", "passing", "stamina", "defense", "physical", "speed"];
  return {
    id,
    name,
    description,
    iconKey,
    offsets: Object.fromEntries(slots.map((slot) => [slot, values[slot] ?? 0])) as Record<PlayerProfileSlot, number>,
    createdAt: fixedTimestamp,
    updatedAt: fixedTimestamp,
  };
}

export function createBoardGameModel(): RosterPlayerModel {
  const base = createCustomRosterPlayerModel(3);
    ...
    attributes: [
      {
        ...
        slot: "attack",
        label: "Strategy",
        description: "Plans ahead and recognizes strong lines of play.",
        order: 0,
      },
      {
        id: "rules-knowledge",
        ...
        description: "Understands systems, interactions and edge cases.",
        order: 1,
      },
      {
        id: "communication",
        slot: "defense",
        label: "Communication",
        description: "Explains, coordinates and helps the group stay aligned.",
        order: 2,
      },
    ],
    presets: [
      preset("strategist", "Strategist", "Plans several turns ahead.", "brain", { attack: 3, passing: 0.5, defense: -1 }),
      preset("rules-expert", "Rules Expert", "Knows systems and edge cases.", "badge-check", { attack: 0, passing: 3, defense: -0.5 }),
      preset("communicator", "Communicator", "Keeps teammates coordinated.", "handshake", { attack: -0.5, passing: 0.5, defense: 3 }),
      preset("wildcard", "Wildcard", "Finds surprising, unconventional solutions.", "sparkles", { attack: 2, passing: -1, defense: 0.5 }),
    ],
    createdAt: fixedTimestamp,
    updatedAt: fixedTimestamp,
  });
...
function extendedFootballModel() {
  const model = createDefaultRosterPlayerModel();
  return normalizeRosterPlayerModel({
    ...model,
    presets: [
      ...model.presets,
      preset("calm-controller", "Calm Controller", "Slows the game down and keeps possession.", "waves", { passing: 2.5, physical: 1, speed: -1.5 }),
      preset("direct-runner", "Direct Runner", "Attacks space quickly and repeatedly.", "footprints", { attack: 2, stamina: 1, speed: 3, defense: -2 }),
    ],
  });
}
...
export const auditFootballRoster = {
  ...
  players: footballNames.map((name, index) => auditPlayer(
    name,
    index,
    index === 1 ? ["playmaker"] : index === 2 ? ["fast", "high-energy"] : [],
  )),
  playerModel: extendedFootballModel(),
  pairingRules: [],
  themeColor: "#4f46e5",
  logo: "",
  createdAt: fixedTimestamp,
...
export const auditBoardGameRoster = {
  id: "audit-roster-board-game",
  name: "Friday Board Game Night",
  players: footballNames.slice(0, 8).map((name, index) => ({
    ...auditPlayer(name, index + 30, index % 3 === 0 ? ["strategist"] : []),
    id: `audit-board-player-${index + 1}`,
  })),
  playerModel: createBoardGameModel(),
  pairingRules: [],
  themeColor: "#0f766e",
  logo: "",
  createdAt: fixedTimestamp,
...
export const auditEmptyRoster = {
  ...
  players: [],
  playerModel: createCustomRosterPlayerModel(3),
  pairingRules: [],
  themeColor: "#ffffff",
  logo: "",
  createdAt: fixedTimestamp,
...
export function createLongContentRoster() {
  const longNames = Array.from({ length: 30 }, (_, index) => (
    index % 5 === 0
  ...
  return {
    ...
    players: longNames.map((name, index) => ({
      ...auditPlayer(name, index + 80, index % 4 === 0 ? ["goal-threat"] : []),
...
export async function seedAuditApp(
  ...
  rosters = [auditFootballRoster, auditBoardGameRoster, auditEmptyRoster],
) {
...
export async function activateAuditControl(
  locator: Locator,
  description: string,
  options: { allowHiddenFallback?: boolean } = {},
) {
  await waitForAuditLocatorToAttach(locator, description);
  const count = await locator.count();
  for (let index = 0; index < count; index += 1) {
    ...
    return { usedHiddenFallback: false };
  }

  if (options.allowHiddenFallback && count > 0) {
    const candidate = locator.first();
    console.warn(`[UX audit] ${description} has no visible entry point at this viewport; opening the hidden control only to capture the resulting state.`);
    await candidate.evaluate((element) => (element as HTMLElement).click());
    return { usedHiddenFallback: true };
  }
